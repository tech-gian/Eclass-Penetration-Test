...
if (isset($mysqlServer) and isset($mysqlUser) and isset($mysqlPassword)) {
	$conn = mysqli_connect($mysqlServer, $mysqlUser, $mysqlPassword, $mysqlMainDb);
	$namesUTF8 = $conn->prepare("SET NAMES utf8");
	$namesUTF8->execute();

	// Old version of mysql_connect
...
if (isset($_SESSION['shib_uname'])) { // authenticate via shibboleth
	...
	if(!empty($submit)) {
		unset($uid);
		$sqlLogin = $conn->prepare("SELECT user_id, nom, username, password, prenom, statut, email, perso, lang
			FROM eclass.user WHERE username=?");
		$sqlLogin->bind_param("s", $uname);
		$sqlLogin->execute();
		$result = $sqlLogin->get_result();
		...
		if (!isset($uid)) {
			...
			$insertQuery = $conn->prepare("INSERT INTO eclass.loginout (loginout.id_user, loginout.ip, loginout.when, loginout.action)
			VALUES (?, ?, NOW(), 'LOGIN')");
			$insertQuery->bind_param("ss", $uid, $REMOTE_ADDR);
			$insertQuery->execute();
		}
...
if (isset($uid) AND !isset($logout)) {
	...
	if (isset($_SESSION['user_perso_active']) and $_SESSION['user_perso_active'] == 'no') {
		if (!check_guest()){
			...
			$guestSQL = $conn->prepare("SELECT code FROM eclass.cours_user, eclass.cours
				WHERE cours.cours_id = cours_user.cours_id AND user_id = ?");
			$guestSQL->bind_param("s", $uid);
			$guestSQL->execute();
			$guestSQL_result = $guestSQL->get_result();

			if (mysqli_num_rows($guestSQL_result) > 0) {
				$sql_row = $guestSQL_result->fetch_assoc();
				$dbname=$sql_row['code'];
...
elseif ((isset($logout) && isset($uid)) OR (1==1)) {
	if (isset($logout) && isset($uid)) {
		$insertQuery = $conn->prepare("INSERT INTO eclass.loginout (loginout.id_user,
