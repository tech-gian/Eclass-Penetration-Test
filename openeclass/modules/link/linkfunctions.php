...
function addlinkcategory($type)
{
	...
	if($type == "category")
	{
		...
		$tbl_categories=trim($tbl_categories);
		$categoryname=trim(q($categoryname));
		$description=trim(q($description));
		$langCategoryAdded=trim(q($langCategoryAdded));
		$langGiveCategoryName=trim(q($langGiveCategoryName));
