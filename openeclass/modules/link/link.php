...
if (isset($action) && ($is_adminOfCourse)) { //allow link management actions only for course admin
	switch($action)
	{
		case "addlink":	if(isset($submitLink) && validate_csrf_token()) {
			if(!addlinkcategory("link"))	// here we add a link
			unset($submitLink);
		}
		break;
		case "addcategory": if(isset($submitCategory) && validate_csrf_token()) {
	...
	if (isset($action) and ($action=="addlink" or $action=="editlink") and !isset($submitLink))
	{
		...
		$tool_content .=  "</b>
          ...
          <tr>
            ...
            <td><input type=\"text\" name=\"urllink\" size=\"53\" value=\"".@q($urllink)."\" class='FormData_InputText' /></td>
          </tr>
          <tr>
            <th class='left'>".$langLinkName." :</th>
            <td><input type=\"text\" name=\"title\" size=\"53\" value=\"".@q($title)."\" class='FormData_InputText' /></td>
          </tr>
          <tr>
            <th class='left'>".$langDescription." :</th>
            <td><textarea rows=\"3\" cols=\"50\" name=\"description\" class='FormData_InputText'>".trim(@q($description))."</textarea></td>
          </tr>
          <tr>
	...
	elseif(isset($action) and ($action=="addcategory" or $action=="editcategory") and !isset($submitCategory))
	{
		...
		$tool_content .=  "</b>
          ...
          <tr>
            ...
            <td><input type=\"text\" name=\"categoryname\" size=\"53\" value=\"".@q($categoryname)."\" class='FormData_InputText' /></td>
          </tr>
          <tr>
            <th class='left'>".$langDescription." :</th>
            <td><textarea rows=\"5\" cols=\"50\" name=\"description\" class='FormData_InputText' >" .
                @q($description)."</textarea></td>
          </tr>
          <tr>
            <th>&nbsp;	</th>
			<input type = 'hidden' name = 'csrf_token' value ='".$_SESSION['csrf_token']."'/>
