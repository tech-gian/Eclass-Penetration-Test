...
if ($is_adminOfCourse) {
	...
	if (isset($mkInvisibl) or isset($mkVisibl)) { 
		if (@$mkInvisibl == true) {
			$sql = $conn->prepare("UPDATE agenda SET visibility = 'i'
				WHERE id=?");
			$sql->bind_param("s", $id);
			$p_sql = $conn->prepare("DELETE FROM agenda WHERE lesson_code = ?
				AND lesson_event_id =?");
			$p_sql->bind_param("ss", $currentCourseID, $id);
		} elseif (@$mkVisibl == true) {
			$sql = "UPDATE agenda SET visibility = 'v' WHERE id='".mysql_real_escape_string($id)."'";
			$sql = $conn->prepare("UPDATE agenda SET visibility = 'v' WHERE id=?");
			$sql->bind_param("s", $id);
			$p_sql = $conn->prepare("SELECT id, titre, contenu, DAY, HOUR, lasting
				FROM agenda WHERE id=?");
			$p_sql->bind_param("s", $id);
			$p_sql->execute();
			$perso_result = $p_sql->get_result();
			$perso_query_result = $perso_result->fetch_assoc();
			$perso_matrix['titre'] = $perso_query_result['titre'];
			$perso_matrix['contenu'] = $perso_query_result['contenu'];
			$perso_matrix['date_selection'] = $perso_query_result['DAY'];
			$perso_matrix['hour'] = $perso_query_result['HOUR'];
			$perso_matrix['lasting'] = $perso_query_result['lasting'];
			// Add all data to the main table.
			$p_sql = $conn->prepare("INSERT INTO agenda (lesson_event_id, titre, contenu, day, hour, lasting, lesson_code)
				VALUES(?,?,?,?,?,?,?)");
			$p_sql->bind_param("sssssss", $perso_query_result['id'], $perso_query_result['titre'], $perso_query_result['contenu'],
				$perso_query_result['DAY'], $perso_query_result['HOUR'], $perso_query_result['lasting'], $currentCourseID);
		}
		$sql->execute();
		$p_sql->execute();
	}
	if (isset($_POST['submit']) && validate_csrf_token()) {
		...
		$date_selection = htmlspecialchars($date_selection);
		$titrhoure = htmlspecialchars($hour);
		$lasting = htmlspecialchars($lasting);
		$titidre = htmlspecialchars($id);


		if(isset($id) && $id) {
			$sql = $conn->prepare("UPDATE agenda
				SET titre=?,
				contenu=?,
				day=?,
				hour=?,
				lasting=?
				WHERE id=?");
			$sql->bind_param("ssssss", $titre, $contenu, $date_selection, $hour, $lasting, $id);

			##[BEGIN personalisation modification]############
			$perso_sql = $conn->prepare("UPDATE $mysqlMainDb.agenda
				SET titre=?,
				contenu=?,
				day = ?,
				hour= ?,
				...
				WHERE lesson_code= ?
				AND lesson_event_id=?");
			$perso_sql->bind_param("sssssss", $titre, $contenu, $date_selection, $hour, $lasting, $currentCourseID, $id);
			...
			$sql = $conn->prepare("INSERT INTO agenda (titre, contenu, day, hour, lasting)
				VALUES (?,?,?,?,?)");
			$sql->bind_param("sssss", $titre, $contenu, $date_selection, $hour, $lasting)
			unset($id);
			unset($contenu);
			unset($titre);
		}
	...
	elseif (isset($delete) && $delete) {
		$sql = $conn->prepare("DELETE FROM $mysqlMainDb.agenda WHERE id=?");
		$sql->bind_param("s", $id);
		$sql->execute();
		$result = $sql->get_result();

		##[BEGIN personalisation modification]############
		$perso_sql = $conn->prepare("DELETE FROM $mysqlMainDb.agenda
			WHERE lesson_code= ?
			AND lesson_event_id=?");
		$perso_sql->bind_param("ss", $currentCourseID, $id);
		$perso_sql->execute();
...
';
	...
	if (isset($id) && $id) {
		$sql = $conn->prepare("SELECT id, titre, contenu, day, hour, lasting FROM agenda WHERE id=?");
		$sql->bind_param("s", $id);
		$sql->execute();
		$result = $sql->get_result():
		...
		$id = $myrow["id"];
		$titre = $myrow["titre"];
		$contenu= $purifier->purify($myrow["contenu"]);
    ...
    <tr>
      ...
      ";
		...
		$tool_content .= "<tr><th class='left'>$langDetail:</th>
			...
			<input type='submit' name='submit' value='$langAddModify' /></td>
			</tr></tbody></table>
			</form><br />";
	}
}
