...
if (!extension_loaded('gd')) {
    ...
    $sql = $conn->prepare("SELECT a.code code, a.intitule intitule
      FROM cours AS a LEFT JOIN cours_user AS b
      ON a.cours_id = b.cours_id
      WHERE b.user_id = ? ORDER BY a.intitule");
    $sql->bind_param("s", $uid);
    $sql->execute();
    $result = $sql->get_result();
    if (mysqli_num_rows($result) > 0) {  // found courses ?
    ...
    $result->free_result();
      ...
      $sql = $conn->prepare("SELECT COUNT(*) AS cnt FROM `$course_code`.actions WHERE user_id = ?");
      $sql->bind_param("s", $uid);
      $sql->execute();
      $result = $sql->get_result();
			while ($row = $result->fetch_assoc()) {
      ...
      $result->free_result();
      $sql = $conn->prepare("SELECT SUM(duration) FROM `$course_code`.actions WHERE user_id = ?");
      $sql->bind_param("s", $uid);
      $sql->execute();
      $result = $sql->get_result();
			list($duration[$course_code]) = $result->fetch_assoc();
...
$sql = $conn->prepare("SELECT * FROM loginout
  WHERE id_user = ? ORDER by idLog DESC LIMIT 10");
