...
if (isset($submit) && isset($changePass) && ($changePass == "do")) {
	...
	$sql = $conn->prepare("SELECT `nom`,`prenom` ,`username`,`email`,`am` FROM `user`WHERE `user_id`=?");
	...
	$sql = $conn->prepare("SELECT `password` FROM `user` WHERE `user_id`=?");
	$sql->bind_param("s", $_SESSION['uid']);
	$sql->execute();
	$result = $sql->get_result();
	$myrow = $result->fetch_assoc();
	...
	if($old_pass == $old_pass_db) {
		...
		$sql->bind_param("ss", $new_pass, $_SESSION['uid']);
		$sql->execute();
