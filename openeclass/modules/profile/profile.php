...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
	...
	$tempSql->bind_param("s", $username_form);
	$tempSql->execute();
	...
	// everything is ok
		...
		$username_form = q($username_form);
		$email_form = q($email_form);
		$am_form = q($am_form);
		$tempSql = $conn->prepare("UPDATE user
			SET nom=?, prenom=?,
			username=?, email=?, am=?,
			perso=?, lang=?
			WHERE user_id=?");
		$tempSql->bind_param("ssssssss", $nom_form, $prenom_form, $username_form, $email_form, $am_form, $persoStatus, $langcode, $_SESSION['uid']);
		$tempSql->execute();
...
if (isset($submit) && isset($ldap_submit) && ($ldap_submit == "ON")) {
	...
	$tempSql = $conn->prepare("UPDATE user SET perso = ?,
		lang = ? WHERE user_id=?");
	$tempSql->bind_param("sss", $persoStatus, $langcode, $_SESSION['uid']);
	$tempSql->execute();
...
$sqlGetInfoUser = $conn->prepare("SELECT nom, prenom, username, password, email, am, perso, lang
	FROM user WHERE user_id=?");
$sqlGetInfoUser->bind_param("s", $uid);
$sqlGetInfoUser->execute();
$result = $sqlGetInfoUser->get_result();
