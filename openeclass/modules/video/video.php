...
$nick=$prenom." ".$nom;
if (  (isset($_POST['submit']) or isset($_POST['edit_submit']) && validate_csrf_token() ) ) {
	if($id) {
		$titre = q($titre);
	...
	elseif (isset($delete)) {
		...
		$tool_content .= "
<form method=\"POST\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?submit=yes\" enctype=\"multipart/form-data\" id=\"insert_form\" onsubmit=\"return checkrequired(this, 'titre');\">
    ...
    <tr>
	  ...
	  <input type=\"submit\" name=\"submit\" value=\"$dropbox_lang[uploadFile]\"></td>
    </tr>
    </tbody>
    </table>
</form>
...
<form method=\"POST\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?submit=yes\" enctype=\"multipart/form-data\" id=\"insert_form\" onsubmit=\"return checkrequired(this, 'titre');\">
    ...
    <tr>
      ...
      <td>
	  <input type = 'hidden' name = 'csrf_token' value ='".$_SESSION['csrf_token']."'/> 
	  <input type=\"submit\" name=\"submit\" value=\"$langAdd\"></td>
    </tr>
    </tbody>
    </table>
</form>
...
if (isset($id)) {
   if($id != "") {
	...
	if ($table_edit == 'videolinks') {
	  	$url= $myrow[1];
          	$titre = q($myrow[2]);
      	  	$description = q($myrow[3]);
      	  	$creator = q($myrow[4]);
      	  	$publisher = q($myrow[5]);
	} elseif ($table_edit == 'video') {
	  	$url= $myrow[2];
          	$titre = q($myrow[3]);
      	  	$description = q($myrow[4]);
      	  	$creator = q($myrow[5]);
      	  	$publisher = q($myrow[6]);
	}
	...
	$tool_content .= "
<form method=\"POST\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?submit=yes\" enctype=\"multipart/form-data\" id=\"edit_form\" onsubmit=\"return checkrequired(this, 'titre');\">
        <table width=\"99%\" class=\"FormData\">
        <tbody>
    	<tr>
          <th class=\"left\" width=\"220\">&nbsp;</th>
