...
if (!isset( $_POST['authors']) || !isset( $_POST['description']))
	...
	if (!$error) {
		...
		if (!$error) {
			...
			// Adding more randomness to the filename
			$filenameArray = explode(".", $dropbox_filename);
			array_pop($filenameArray);
			$tempFilename = implode(".", $filenameArray);
			$tempFilename = uniqid("_._", true);
			$dropbox_filename = $tempFilename . "." . $format . ".txt";
			/////////////////////////////////////////
	...
	if (!preg_match($dropbox_cnf["mailingZipRegexp"], $mailing_title, $nameParts))
	{
		...
		if (!$error)
		{
			...
			if ( mysqli_num_rows($result) > 0)
			{
				...
				{
					$remainingUsers .= ', ' . htmlspecialchars($res['nom'].' '.$res['prenom']);
				}
			...
			if ( isset( $_GET['mailingSend']))
			{
				...
				$sql = $conn->prepare("UPDATE `".$dropbox_cnf["fileTbl"]."`
					SET filesize = '0'
					, uploadDate = ?, lastUploadDate = ?
					WHERE id=?");
				$sql->bind_param("sss", $sendDT, $sendDT, $mailing_item->id);
				$sql->execute() or die($dropbox_lang["queryError"]);
				$result = $sql->get_result();
			}
