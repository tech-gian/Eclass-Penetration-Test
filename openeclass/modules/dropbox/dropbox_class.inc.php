...
class Dropbox_Work {
	...
	function _createNewWork ($uploaderId, $title, $description, $author, $filename, $filesize) {
		...
		*/
		...
		if ($GLOBALS['language'] == 'greek') {
			$sql = $conn->prepare("SELECT id, DATE_FORMAT(uploadDate, '%d-%m-%Y / %H:%i')
				FROM `$currentCourseID`.`".$dropbox_cnf["fileTbl"]."` 
				WHERE filename = ?");
			$sql->bind_param("s", $this->filename);
		} else {
			$sql = $conn->prepare("SELECT id, DATE_FORMAT(uploadDate, '%Y-%m-d% / %H:%i')
				FROM `$currentCourseID`.`".$dropbox_cnf["fileTbl"]."` 
				WHERE filename = ?");
			$sql->bind_param("s", $this->filename);
		}
		...
		*/
		if ($this->isOldWork) {
			...
			$sql = $conn->prepare("UPDATE `$currentCourseID`.`".$dropbox_cnf["fileTbl"]."`
				SET filesize = ?
				, title = ?
				, description = ?
				, author = ?
				, lastUploadDate = ?
				WHERE id=?");
			$sql->bind_param("ssssss", $this->filesize, $this->title, $this->description, $this->author, $this->lastUploadDate, $this->id);
			$sql->execute();
			$result = $sql->get_result();
		} else {
			$this->uploadDate = $this->lastUploadDate;
			$sql = $conn->prepare("INSERT INTO `$currentCourseID`.`".$dropbox_cnf["fileTbl"]."`
			...
			$sql->bind_param("ssssssss", $this->uploaderId, $this->filename, $this->filesize, $this->title,
				$this->description, $this->author, $this->uploadDate, $this->lastUploadDate);
			$sql->execute();
			$result = $sql->get_result();
			$this->id = mysqli_insert_id($conn); //get automatically inserted id
		}
		...
		*/
		$sql = $conn->prepare("INSERT INTO `$currentCourseID`.`".$dropbox_cnf["personTbl"]."`
			(fileId, personId)
			VALUES (?
					, ?
					)");
		$sql->bind_param("ss", $this->id, $this->uploaderId);
		$sql->execute();
		$result = $sql->get_result();  //if work already exists no error is generated
	}
	...
	function _createExistingWork ($id) {
		...
		*/
	if ($GLOBALS['language'] == 'greek') {
		$sql = $conn->prepare("SELECT uploaderId, filename, filesize, title, description, author,
...
class Dropbox_SentWork extends Dropbox_Work {
	...
	function _createNewSentWork ($uploaderId, $title, $description, $author, $filename, $filesize, $recipientIds) {
		...
		*/
		foreach ($this->recipients as $rec) {	
			$sql = $conn->prepare("INSERT INTO `$currentCourseID`.`".$dropbox_cnf["postTbl"]."`
				(fileId, recipientId)
				VALUES (?, ?)");
			$sql->bind_param("ss", $this->id, $rec['id']);
			$sql->execute();
			$result = $sql->get_result();  //if work already exists no error is generated
						
			//insert entries into person table
			$sql = $conn->prepare("INSERT INTO `$currentCourseID`.`".$dropbox_cnf["personTbl"]."` (fileId, personId)
				VALUES (?, ?)");
			$sql->bind_param("ss", $this->id, $rec['id']);
	...
	function _createExistingSentWork  ($id) {
		...
		*/
		...
		$sql = $conn->prepare("SELECT recipientId
			FROM `$currentCourseID`.`".$dropbox_cnf["postTbl"]."`
			WHERE fileId=?");
		$sql->bind_param("s", $id);
		$sql->execute();
		$result = $sql->get_result();
...
class Dropbox_Person {
	...
	function Dropbox_Person ($userId, $isCourseAdmin, $isCourseTutor) {
		...
		*/
		$sql = $conn->prepare("SELECT r.fileId FROM
			`$currentCourseID`.`".$dropbox_cnf["postTbl"]."` r
			, `$currentCourseID`.`".$dropbox_cnf["personTbl"]."` p
			WHERE r.recipientId = ?
			AND r.recipientId = p.personId
			AND r.fileId = p.fileId");
		$sql->bind_param("s", $this->userId);
		$sql->execute();
		$result = $sql->get_result();
		while ($res = $result->fetch_assoc()) {
		...
		*/
		$sql = "SELECT f.id FROM `".$dropbox_cnf["fileTbl"]."` f, `".$dropbox_cnf["personTbl"]."` p 
				...
				AND f.id = p.fileId";
		$sql = $conn->prepare("SELECT f.id FROM `$currentCourseID`.`".$dropbox_cnf["fileTbl"]."` f,
			`$currentCourseID`.`".$dropbox_cnf["personTbl"]."` p
			WHERE f.uploaderId = ?
			AND f.uploaderId = p.personId
			AND f.id = p.fileId");
	...
	function deleteAllReceivedWork () {
		...
		*/
		...
		foreach ($this->receivedWork as $w) {
			$tempSql = $conn->prepare("DELETE FROM `$currentCourseID`.`".$dropbox_cnf["personTbl"]."`
				WHERE personId=? AND fileId=?");
			$tempSql->bind_param("ss", $this->userId, $w->id);
			$tempSql->execute();
		}
	...
	function deleteReceivedWork ($id) {
		...
		*/
		...
		$tempSql = $conn->prepare("DELETE FROM `$currentCourseID`.`".$dropbox_cnf["personTbl"]."`
			WHERE personId=? AND fileId=?");
		$tempSql->bind_param("ss", $this->userId, $id);
