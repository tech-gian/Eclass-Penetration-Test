...
function delete_users() {
	global $cours_id, $langUsersDeleted, $conn;

	$tempSql = $conn->prepare("DELETE FROM cours_user WHERE cours_id = ? and statut <> '1'");
...
function delete_announcements() {
	global $cours_id, $langAnnDeleted, $conn;
...
function delete_agenda() {
	global $langAgendaDeleted, $currentCourseID, $mysqlMainDb, $conn;

	$tempSql = $conn->prepare("DELETE FROM agenda");
	$tempSql->execute();

	##[BEGIN personalisation modification]############
	$tempSql = $conn->prepare("DELETE FROM ".$mysqlMainDb.".agenda WHERE lesson_code=?");
...
function hide_doc()  {
	global $langDocsDeleted, $conn;

	$tempSql = $conn->prepare("UPDATE document SET visibility='i'");
	$tempSql->execute();
	return "<p>$langDocsDeleted</p>";
}

function hide_work()  {
	global $langWorksDeleted, $conn;

	$tempSql = $conn->prepare("UPDATE assignments SET active=0");
	$tempSql->execute();
