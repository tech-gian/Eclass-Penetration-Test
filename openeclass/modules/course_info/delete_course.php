...
if($is_adminOfCourse) {
	if(isset($delete)) {
		...
		$tempSql = $conn->prepare("DROP DATABASE `$currentCourseID`");
		$tempSql->execute();
		$tempSql = $conn->prepare("DELETE FROM `$mysqlMainDb`.cours WHERE code=?");
		$tempSql->bind_param("s", $currentCourseID);
		$tempSql->execute();
		$tempSql = $conn->prepare("DELETE FROM `$mysqlMainDb`.cours_user WHERE cours_id=?");
		$tempSql->bind_param("s", $cours_id);
		$tempSql->execute();
		$tempSql = $conn->prepare("DELETE FROM `$mysqlMainDb`.cours_faculte WHERE code=?");
		$tempSql->bind_param("s", $currentCourseID);
		$tempSql->execute();
		$tempSql = $conn->prepare("DELETE FROM `$mysqlMainDb`.annonces WHERE cours_id=?");
		$tempSql->bind_param("s", $cours_id);
		$tempSql->execute();
