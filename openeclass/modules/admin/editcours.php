...
if (isset($c)) {
  ...
  $tempSql = $conn->prepare("SELECT * FROM cours WHERE code = ?");
  ...
  $tempSql = $conn->prepare("SELECT code,intitule,doc_quota,video_quota,group_quota,dropbox_quota
    FROM cours WHERE code=?");
  $tempSql->bind_param("s", $c);
  $tempSql->execute();
  $tempResult = $tempSql->get_result();
  $q = $tempResult->fetch_assoc();
