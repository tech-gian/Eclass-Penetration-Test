...
if((!empty($u)) && ctype_digit($u) )	// validate the user id
{
	...
	{
		$tempSql = $conn->prepare("SELECT nom, prenom, username, password, email, phone, department, registered_at, expires_at, statut, am FROM user WHERE user_id = ?");
...
<form name='edituser' method='post' action='" . htmlspecialchars($_SERVER['PHP_SELF']) . "'>
  ...
  <tr>
    <th width='220'>&nbsp;</th>
    <td><b>$langEditUser ".q($info['username'])."</b></td>
  </tr>
  <tr>
 ...
 <tr>
  ...
  <tr>
    <td colspan='2'>&nbsp;</td>
		...
		$tempSql = $conn->prepare("SELECT nom, prenom, username FROM user WHERE user_id = ?");
		$tempSql->bind_param("s", $u);
		$tempSql->execute();
		$sql = $tempSql->get_result();
		$tempSql = $conn->prepare("SELECT a.code, a.intitule, b.reg_date, b.statut, a.cours_id
		...
		if (mysqli_num_rows($sql) > 0)
		{
			...
			for ($j = 0; $j < mysqli_num_rows($sql); $j++)
			{
				...
				$tool_content .= "<tr>
						<td>".htmlspecialchars($logs['code'])."</td>
						<td>".htmlspecialchars($logs['intitule'])."</td><td align='center'>";
				if ($logs['ref_date'] == '0000-00-00')
				...
				switch ($logs['statut'])
				{
					...
					case 5:
						...
						$code = $logs['code'];
						$tool_content .= "</td><td align='center'>
						<a href='unreguser.php?u=$u&amp;c=$code'>
		...
		else
		{
			...
			if ($u > 1)
			{
				if (isset($logs)) {
					$code = $logs['code'];
					$tool_content .= "<center>
					<a href=\"unreguser.php?u=$u&c=$code\">$langDelete</a></center>";
				}
				else
					$tool_content .= "<center>
		...
		$tempSql = $conn->prepare("SELECT username FROM user WHERE username=?");
		$tempSql->bind_param("s", $username);
		$tempSql->execute();
		$username_check = $tempSql->get_result();
		$nums = mysqli_num_rows($username_check);
	...
	draw($tool_content, 3, ' ', $head_content);
 	 ...
 	 elseif(isset($user_exist) AND $user_exist == TRUE) {
		...
		if($registered_at>$expires_at) {
			...
			$sql = $conn->prepare("UPDATE user SET nom = ?, prenom = ?,
				username = ?, email = ?, 
				statut = ?, phone=?,
				department = ?, expires_at=?,
				am = ? WHERE user_id = ?");
			$sql->bind_param("ssssssssss", $lname, $fname, $username, $email, $newstatut, $phone, $department, $expires_at, $am, $u);
                        if ($sql->execute()) {
							$num_update = $sql->affected_rows;
							if ($num_update == 1) {
