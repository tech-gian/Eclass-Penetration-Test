...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		// restore request
		$tempSql = $conn->prepare("UPDATE prof_request set status='1', date_closed=NULL WHERE rid=?");
		...
		$tool_content .= "<tbody>";
		$tempSql = $conn->prepare("SELECT rid,profname,profsurname,profuname,profemail,proftmima,
			profcomm,date_open,date_closed,comment
			FROM prof_request WHERE (status = 2 AND statut = ?)");
		$tempSql->bind_param("s", $list_statut);
		$tempSql->execute();
		$sql = $tempSql->get_result();
		$k = 0;
...
// ----------------------------------
	...
	// restore request
		...
		$tempSql = $conn->prepare("SELECT rid,profname,profsurname,profuname,profemail,
			proftmima,profcomm,date_open,date_closed,comment
			FROM prof_request WHERE (status = 3 AND statut = ?)");
		$tempSql->bind_param("s", $list_statut);
		$tempSql->execute();
		$sql = $tempSql->get_result();
		$k = 0;
...
// ---------------------------------
	...
	$tempSql = $conn->prepare("SELECT rid,profname,profsurname,profuname,profemail,proftmima,
		profcomm,date_open,date_closed,comment
		FROM prof_request WHERE (status = 0 AND statut = ?)");
	$tempSql->bind_param("s", $list_statut);
	$tempSql->execute();
	$sql = $tempSql->get_result();
	...
	for ($j = 0; $j < mysqli_num_rows($sql); $j++) {
		...
		$tool_content .= "\n    <td>".htmlspecialchars($req['profname'])."&nbsp;".htmlspecialchars($req['profsurname'])."</td>";
		for ($i = 3; $i < mysqli_num_fields($sql) - 3; $i++) {
			if ($i == 4 and $req[$i] != "") {
				$tool_content .= "\n    <td><a href=\"mailto:".
...
// ------------------------------
	...
	switch($close) {
	case '1':
		$tempSql = $conn->prepare("UPDATE prof_request set status='2', date_closed=NOW() WHERE rid=?");
		$tempSql->bind_param("s", $id);
		$tempSql->execute();
		$sql = $tempSql->get_result();
		if ($list_statut == 1) {
	...
	case '2':
		...
		if(!empty($submit)) {
			...
			if (!empty($comment)) {
				$sql = $conn->prepare("UPDATE prof_request set status = '3',
					date_closed = NOW(),
					comment = ? WHERE rid = ?");
				$sql->bind_param("ss", $comment, $id);
				if ($sql->execute()) {
...
$langEmail: $emailhelpdesk";
			...
			// display the form
			$tempSql = $conn->prepare("SELECT comment, profname, profsurname, profemail, statut
...
else
{
	...
	$tempSql = $conn->prepare("SELECT rid,profname,profsurname,profuname,profemail,proftmima, 
		...
		FROM prof_request WHERE (status = 1 AND statut = ?)");
	$tempSql->bind_param("s", $list_statut);
	$tempSql->execute();
	$sql = $tempSql->get_result();
	$k = 0;
