...
{
	$u = (int)$u;
	$tempSql = $conn->prepare("SELECT nom, prenom, username, password, email, phone, department, registered_at, expires_at FROM user WHERE user_id = ?");
	...
	<thead>
	<tr><th width=\"220\">&nbsp;</th>
	<td><b>$langUserStats</b>: $info[username]</td>
	</tr>";
	...
	if (mysqli_num_rows($sql) > 0) {
		...
		for ($j = 0; $j < mysqli_num_rows($sql); $j++) {
			...
			<img style='border:0px; padding-top:3px;' src='${urlServer}/template/classic/img/arrow_grey.gif' title='bullet'></td>
         		<td align=\"left\">".htmlspecialchars($logs['code'])." (".htmlspecialchars($logs['intitule']).")</td>
         		<td><div align=\"center\">";
			switch ($logs['statut']) {
				case 1:
				...
				case 5:
					...
					<a href=\"unreguser.php?u=$u&un=$info[username]&c=$logs[code]\">
		...
		$tool_content .= "$langNoStudentParticipation ";
		  if ($u > 1) {
				if (isset($logs))
			    $tool_content .= "<center><a href=\"unreguser.php?u=$u&un=$info[username]&c=$logs[code]\">$langDelete</a></center>";
				else
			    $tool_content .= "<center><a href=\"unreguser.php?u=$u&un=$info[username]&c=\">$langDelete</a></center>";
		  }
		  else
		  {
...
if (!extension_loaded('gd')) {
	...
	if (mysqli_num_rows($result) > 0) {
		...
		foreach ($course_codes as $course_code) {
			...
			$tempSql = $conn->prepare("SELECT COUNT(*) AS cnt FROM actions WHERE user_id = ?");
			$tempSql->bind_param("s", $u);
			$tempSql->execute();
			$result = $tempSql->get_result();
			while ($row = $result->fetch_assoc()) {
...
$sql = $conn->prepare("SELECT * FROM loginout WHERE id_user = ? ORDER by idLog DESC LIMIT 15");
$sql->bind_param("s", $u);
$sql->execute();

$leResultat = $sql->get_result();
    $tool_content .= "
  <tr>
    <th class=\"left\" width=\"220\" valign=\"top\">$langLastUserVisits $info[username]</th>
    <td>\n";
$tool_content .= "
      <table width=\"99%\">
      <thead>
...
$i=0;
while ($leRecord = $leResultat->fetch_assoc()) {
	$when = $leRecord["when"];
