...
{
	$user_surname_qry = " nom LIKE CONCAT(?, '%')";
	array_push($criteria_array, $user_surname);
	$criteria++;
}
else
{
...
if(!empty($user_firstname))
{
	...
	$criteria++;
	$user_firstname_qry .= " prenom LIKE CONCAT(?, '%')";
	array_push($criteria_array, $user_firstname);
}
else
{
	$user_firstname_qry = "";
...
if(!empty($user_username))
{
	...
	$criteria++;
	$user_username_qry .= " username LIKE CONCAT(?, '%')";
	array_push($criteria_array, $user_username);
}
else
{
	$user_username_qry = "";
...
if(!empty($user_am))
{
	...
	$criteria++;
	$user_am_qry .= " am=?";
	array_push($criteria_array, $user_am);
}
else
{
	$user_am_qry = "";
...
if(!empty($user_type))
{
	...
	$criteria++;
	$user_type_qry .= " statut=?";
	array_push($criteria_array, $user_type);
}
else
{
	$user_type_qry = "";
...
if(!empty($user_email))
{
	...
	$criteria++;
	$user_email_qry .= " email LIKE CONCAT(?, '%')";
	array_push($criteria_array, $user_email);
}
else
{
	$user_email_qry = "";
...
if($view == 3) { // users per course
	...
	// Bind the parameters, from user_input
	$tempSql = $conn->prepare($qry);

	$paramTypes = str_repeat("s", count($criteria_array));
	$bindParams = array($paramTypes);
	foreach ($criteria_array as $key => $value) {
		$bindParams[] =& $criteria_array[$key];
	}
	call_user_func_array(array($tempSql, 'bind_param'), $bindParams);
}
