...
// Manage list limits
$tempSql = $conn->prepare("SELECT COUNT(*) AS cnt FROM cours");
$tempSql->execute();
$tempResult = $tempSql->get_result();
$countcourses = $tempResult->fetch_assoc();
...
if (isset($search) && $search=="yes") {
	...
	if(!empty($searchtitle)) {
		$searchcours[] = "intitule LIKE CONCAT('%', ?, '%')";
		array_push($criteria, $searchtitle);
	}
	if(!empty($searchcode)) {
		$searchcours[] = "code LIKE CONCAT('%', ?, '%')";
		array_push($criteria, $searchcode);
	}
	if ($searchtype != "-1") {
		$searchcours[] = "visible = ?";
		array_push($criteria, $searchtype);
	}
	if($searchfaculte != "0") {
		$searchcours[] = "faculte = ?";
		array_push($criteria, $searchfaculte);
	}
	$query=join(' AND ',$searchcours);
	if (!empty($query)) {
		$tempSql = $conn->prepare("SELECT faculte, code, intitule, titulaires, visible, cours_id FROM cours 
			WHERE $query ORDER BY faculte");
		$paramTypes = str_repeat("s", count($criteria));
		$bindParams = array($paramTypes);
		foreach ($criteria as $key => $value) {
			$bindParams[] =& $criteria[$key];
		}
		call_user_func_array(array($tempSql, 'bind_param'), $bindParams);

		$tempSql->execute();
		$sql = $tempSql->get_result();
		$caption .= "$langFound ".mysqli_num_rows($sql)." $langCourses ";
	} else {
		$tempSql = $conn->prepare("SELECT faculte, code, intitule,titulaires, visible, cours_id FROM cours 
			ORDER BY faculte");
		$tempSql->execute();
...
// Normal list, no search, select all courses
	...
	$tempSql = $conn->prepare("SELECT COUNT(*) FROM cours");
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$a = $tempResult->fetch_assoc();
	$caption .= "".$langManyExist.": <b>".$a[0]." $langCourses</b>";
	$tempSql = $conn->prepare("SELECT faculte, code, intitule, titulaires, visible, cours_id FROM cours 
		ORDER BY faculte,code LIMIT ?,".$listsize."");
	$tempSql->bind_param("s", $limit);
	$tempSql->execute();
	$sql = $tempSql->get_result();
