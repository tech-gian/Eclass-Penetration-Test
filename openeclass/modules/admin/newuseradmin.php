...
if($submit) {
	...
	$tempSql = $conn->prepare("SELECT username FROM `$mysqlMainDb`.user 
		WHERE username=?");
	$tempSql->bind_param("s", $uname);
	$tempSql->execute();
	$username_check = $tempSql->get_result();
	$user_exist = (mysqli_num_rows($username_check) > 0);
	...
	if (!$all_set) {
		...
		$tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.user
			(nom, prenom, username, password, email, statut, department, am, registered_at, expires_at,lang)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$tempSql->bind_param("sssssssssss", $nom_form, $prenom_form, $uname, $password_encrypted, $email_form, $pstatut,
			$depid, $comment, $registered_at, $expires_at, $proflanguage);
		$tempSql->execute();
		$inscr_user = $tempSql->get_result();

		// close request
	  	$rid = intval($_POST['rid']);
		$tempSql = $conn->prepare("UPDATE prof_request set status = 2, date_closed = NOW() WHERE rid = ?");
		$tempSql->bind_param("s", $rid);
		$tempSql->execute();
...
";
	...
	if (isset($id)) { // if we come from prof request
		$tempSql = $conn->prepare("SELECT profname, profsurname, profuname, profemail, proftmima, comment, lang, statut 
