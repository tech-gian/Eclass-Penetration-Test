...
function create_user($statut, $uname, $nom, $prenom, $email, $depid, $am, $phone, $lang, $send_mail)
{
        ...
        $tempSql = $conn->prepare("SELECT * FROM user WHERE username = ?");
        $tempSql->bind_param("s", $uname);
        $tempSql->execute();
        $req = $tempSql->get_result();
        ...
        $tempSql = $conn->prepare("INSERT INTO user
                (nom, prenom, username, password, email, statut, department, registered_at, expires_at, lang, am, phone)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $tempSql->bind_param("ssssssssssss", $nom, $prenom, $uname, $password_encrypted, $email, $statut, $depid,
                $registered_at, $expires_at, $lang, $am, $phone);
        $tempSql->execute();
        $req = $tempSql->get_result();
        $id = $tempSql->insert_id;
...
function create_username($statut, $depid, $nom, $prenom, $prefix)
{
        ...
        $tempSql = $conn->prepare("SELECT username FROM user
                WHERE username LIKE ?
                ORDER BY username DESC LIMIT 1");
        $likeValue = $prefix . $wildcard;
        $tempSql->bind_param("s", $likeValue);
        $tempSql->execute();
        $req = $tempSql->get_result();
        if (mysqli_num_rows($req) > 0) {
                list($last_uname) = $req->fetch_assoc();
...
function register($uid, $course_code)
{
        ...
        $tempSql = $conn->prepare("SELECT code, cours_id FROM cours WHERE code=? OR fake_code=?");
        $tempSql->bind_param("ss", $code, $code);
        $tempSql->execute();
        $req = $tempSql->get_result();
        if (mysqli_num_rows($req) > 0) {
                list($code, $cid) = $req->fetch_assoc();
                $tempSql1 = $conn->prepare("INSERT INTO cours_user SET cours_id = ?, user_id = ?, statut = 5,
                        team = 0, tutor = 0, reg_date = NOW()");
                $tempSql1->bind_param("ss", $cid, $uid);
                $tempSql1->execute();
