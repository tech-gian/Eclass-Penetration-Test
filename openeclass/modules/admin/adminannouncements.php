...
if (isset($_GET['delete'])) {
        ...
        $tempSql = $conn->prepare("DELETE FROM admin_announcements WHERE id=?");
        ...
        $tempSql = $conn->prepare("SELECT * FROM admin_announcements WHERE id=?");
        $tempSql->bind_param("s", $id);
        $tempSql->execute();
        $result = $tempSql->get_result();
	...
	// submit announcement command
        if (isset($_POST['id'])) {
                // modify announcement
                $id = intval($_POST['id']);
                $tempSql = $conn->prepare("UPDATE admin_announcements
                        SET gr_title = ?, gr_body = ?, gr_comment = ?,
                        en_title = ?, en_body = ?, en_comment = ?,
                        visible = ?, date = NOW()
                        WHERE id = ?");
                $tempSql->bind_param("ssssssss", $title, $newContent, $comment, $title_en, $newContent_en, $comment_en, $visible, $id);
                $tempSql->execute();
                $message = $langAdminAnnModify;
        } else {
                // add new announcement
                $tempSql = $conn->prepare("INSERT INTO admin_announcements
                        SET gr_title = ?, gr_body = ?, gr_comment = ?,
                        en_title = ?, en_body = ?, en_comment = ?,
                        visible = ?, date = NOW()");
                $tempSql->bind_param("sssssss", $title, $newContent, $comment, $title_en, $newContent_en, $comment_en, $visible);
                $tempSql->execute();
...
if ($displayAnnouncementList == true) {
        $tempSql = $conn->prepare("SELECT * FROM admin_announcements ORDER BY id DESC");
        $tempSql->execute();
        $result = $tempSql->get_result();
        $announcementNumber = mysqli_num_rows($result);
        if (@$addAnnouce != 1) {
