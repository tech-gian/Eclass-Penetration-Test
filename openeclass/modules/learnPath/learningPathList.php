...
if ($is_adminOfCourse) {
	...
	if (isset($_REQUEST['cmd'])) {
		...
		// ACCESSIBILITY COMMAND
			...
			case "create" :
				// create form sent
				if( isset($_POST["newPathName"]) && $_POST["newPathName"] != "" && validate_csrf_token()) {
					...
					if($num == 0) { // "name" doesn't already exist
                        ...
                        $_POST['newPathName'] = q($_POST['newPathName']);
                        $_POST['newComment'] = q($_POST['newComment']);
    ...
    <tr>
      <th class='left'>&nbsp;</th>
      <td><input type = 'hidden' name = 'csrf_token' value ='".$_SESSION['csrf_token']."'/>
      <input type='hidden' name='cmd' value='create'>
      <input type='submit' value='$langCreate'></input></td>
    </tr>
    </tbody>
    </table>
    </form>";
