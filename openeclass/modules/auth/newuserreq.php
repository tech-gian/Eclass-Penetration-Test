...
if($submit) {
  ...
  while ($myusername = $username_check->fetch_assoc()) {
    $user_exist=$myusername['username'];
  }
...
// register user
    ...
    $tempSql = $conn->prepare("SELECT id FROM faculte WHERE name=?");
    $tempSql->bind_param("s", $department);
    $tempSql->execute();
    ...
    $tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.user
      ...
      VALUES (?, ?, ?, ?, ?, '5', ?, ?, ?, ?)");
    $tempSql->bind_param("sssssssss", $nom_form, $prenom_form, $uname, $password_encrypted,
      $email_form, $dep['id'], $registered_at, $expires_at, $lang);
    $tempSql->execute();
    $inscr_user = $tempSql->get_result();

    // close request
    $rid = intval($_POST['rid']);
    $tempSql = $conn->prepare("UPDATE prof_request set status = '2',
      date_closed = NOW() WHERE rid = ?");
...
$tool_content .= "<table width=\"99%\"><tbody>
  ...
  $tempSql = $conn->prepare("SELECT name FROM faculte order by id");
  $tempSql->execute();
