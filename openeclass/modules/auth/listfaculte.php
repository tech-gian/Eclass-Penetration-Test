...
$tempSql = $conn->prepare("SELECT id, name, code FROM faculte ORDER BY name");
$tempSql->execute();
...
if (isset($result))  {
	...
	while ($fac = $result->fetch_assoc()) {
		...
		$tempSql = $conn->prepare("SELECT COUNT(*) FROM cours_faculte WHERE facid=?");
		$tempSql->bind_param("s", $fac['id']);
		$tempSql->execute();
		$n = $tempSql->get_result();
		$r = $n->fetch_assoc();
