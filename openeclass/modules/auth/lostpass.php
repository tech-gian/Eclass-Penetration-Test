...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$hash = $_REQUEST['h'];
	$tempSql = $conn->prepare("SELECT `user_id`, `hash`, `password`, `datetime` FROM passwd_reset
		WHERE `user_id` = ? AND `hash` = ?
		AND TIME_TO_SEC(TIMEDIFF(`datetime`,NOW())) < 3600");
	$tempSql->bind_param("ss", $userUID, $hash);
	$tempSql->execute();
	...
	if (mysqli_num_rows($res) == 1) {
		...
		$sql->bind_param("ss", $myrow['hash'], $myrow['user_id']);
		if($sql->execute()) {
			//send email to the user of his new pass (not hashed)
			$tempSql = $conn->prepare("SELECT `email` FROM user WHERE `user_id` = ?");
			$tempSql->bind_param("s", $myrow['user_id']);
			$tempSql->execute();
			$res = $tempSql->get_result();
			$myrow2 = $res->fetch_assoc();
			...
			$tempSql = $conn->prepare("DELETE FROM `passwd_reset` WHERE `user_id` = ?");
			$tempSql->bind_param("s", $myrow['user_id']);
			$tempSql->execute();
			// delete passws_reset entries older from 2 days
			$tempSql = $conn->prepare("DELETE FROM `passwd_reset` 
				WHERE DATE_SUB(CURDATE(),INTERVAL 2 DAY) > `datetime`");
			$tempSql->execute();
		}
	...
	$tempSql = $conn->prepare("SELECT user_id, nom, prenom, username, password, statut FROM user
		WHERE email = ? AND BINARY username = ?");
	$tempSql->bind_param("ss", $email, $userName);
	$tempSql->execute();
	$res = $tempSql->get_result();
	...
	if (mysqli_num_rows($res) == 1) {
		...
		while ($s = $res->fetch_assoc()) {
			...
			if($is_editable) {
				...
				$sql = $conn->prepare("INSERT INTO `passwd_reset` (`user_id`, `hash`, `password`, `datetime`) VALUES (?, ?, ?, NOW())");
				$sql->bind_param("sss", $s['user_id'], md5($new_pass), $new_pass);
				$sql->execute();
