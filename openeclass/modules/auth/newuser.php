...
if (!isset($submit)) {
	...
	$tempSql = $conn->prepare("SELECT name, id FROM faculte ORDER BY id");
	$tempSql->execute();
	$deps = $tempSql->get_result();
	$num = mysqli_num_rows($deps);
	...
	// check if the username is already in use
		$q2 = $conn->prepare("SELECT username FROM `$mysqlMainDb`.user WHERE username=?");
		$q2->bind_param("s", $uname);
		$q2->execute();
		$username_check = $q2->get_result();
		if ($myusername = $username_check->fetch_assoc()) {
  ...
  $nom_form = q($nom_form);
	$prenom_form = q($prenom_form);
	$uname = q($uname);
	$password_encrypted = q($password_encrypted);
	$email = q($email);
	$department = q($department);
	$am = q($am);
 
	$q1 = $conn->prepare("INSERT INTO `$mysqlMainDb`.user
		(nom, prenom, username, password, email, statut, department, am, registered_at, expires_at, lang)
		VALUES (?, ?, ?, ?, ?,'5',?,?,?,?,?)");
	$q1->bind_param("ssssssssss", $nom_form, $prenom_form, $uname, $password_encrypted, $email, $department, $am, $registered_at, $expires_at, $lang);
	$q1->execute();
	$last_id = $conn->insert_id;
	$tempSql = $conn->prepare("SELECT user_id, nom, prenom FROM `$mysqlMainDb`.user WHERE user_id=?");
	$tempSql->bind_param("s", $last_id);
	$tempSql->execute();
	...
	$tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.loginout (loginout.idLog, loginout.id_user, loginout.ip, loginout.when, loginout.action)
		VALUES ('', ?, ?, NOW(), 'LOGIN')");
	$tempSql->bind_param("ss", $uid, $REMOTE_ADDR);
	$tempSql->execute();
	$_SESSION['uid'] = $uid;
	$_SESSION['statut'] = 5;
