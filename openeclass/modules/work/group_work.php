...
function submit_work($uid, $id, $file) {
	...
	$tempSql = $conn->prepare("SELECT filename FROM group_documents WHERE path = ?");
	$tempSql->bind_param("s", autoquote($file));
	$tempSql->execute();
	...
	if (copy($source, "$workPath/$destination")) {
		$tempSql = $conn->prepare("INSERT INTO assignment_submit (uid, assignment_id, submission_date,
			submission_ip, file_path, file_name, comments, group_id)
			VALUES (?, ?, NOW(), ?, ?, ?, ?, ?)");
		$tempSql->bind_param("sssssss", $uid, $id, $_SERVER['REMOTE_ADDR'], $destination,
			quote($original_filename), autoquote($_POST['comments']), $group);
		$tempSql->execute();
