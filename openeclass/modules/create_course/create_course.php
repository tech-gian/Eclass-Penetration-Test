...
if (isset($_POST['back1']) or !isset($_POST['visit'])) {
   ...
   // display form
	...
	<td>";
	$tempSql = $conn->prepare("SELECT department FROM user WHERE user_id=?");
	$tempSql->bind_param("s", $uid);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	list($homefac) = $tempResult->fetch_assoc();
	$tempSql = $conn->prepare("SELECT id, name FROM faculte order by id");
	$tempSql->execute();
	$facs = $tempSql->get_result();
	while ($n = $facs->fetch_assoc()) {
		$fac[$n['id']] = $n['name'];
	}
...
if (isset($_POST['create_course']) && validate_csrf_token()) {
        ...
        if (! (mkdir("../../courses/$repertoire", 0777) and
			...
			// Create empty index.php (so no files can be shown) in each folder
			$folders = array(
				"../../courses/$repertoire/image",
				"../../courses/$repertoire/document",
				"../../courses/$repertoire/dropbox",
				"../../courses/$repertoire/page",
				"../../courses/$repertoire/work",
				"../../courses/$repertoire/group",
				"../../courses/$repertoire/temp",
				"../../courses/$repertoire/scormPackages",
				"../../video/$repertoire"
			);
			foreach ($folders as $folder) {
				$fp = fopen("$folder/index.html", "w");
				fwrite($fp, "<h1>You don't have permissions to see the contents of this folder.</h1>");
				fclose($fp);
			}
			////////////////////////////////////////////////////////////////////
		}
		...
		$tempSql = $conn->prepare("INSERT INTO cours SET
			code = ?,
			languageCourse = ?,
			intitule = ?,
			description = ?,
			...
			faculteid = ?,
			first_create = NOW()");
		$tempSql->bind_param("ssssssssssss", $code, $language, $intitule, $description, $course_addon, $course_keywords,
			$facname, $formvisible, $titulaires, $code, $type, $facid);
		$tempSql->execute();
        $new_cours_id = $conn->insert_id;
		$tempSql = $conn->prepare("INSERT INTO cours_user SET
			cours_id = ?,
			user_id = ?,
			statut = '1',
			tutor='1',
			reg_date = CURDATE()");
		$tempSql->bind_param("ss", $new_cours_id, $uid);
		$tempSql->execute();

		$tempSql = $conn->prepare("INSERT INTO cours_faculte SET
			faculte = ?,
			code = ?,
			facid = ?");
		$tempSql->bind_param("sss", $faculte, $repertoire, $facid);
		$tempSql->execute();
