...
if (isset($post_id) && $post_id) {
	...
	$sql = $conn->prepare("SELECT f.forum_type, f.forum_name, f.forum_access, t.topic_title
		FROM `$currentCourseID`.forums f, `$currentCourseID`.topics t, `$currentCourseID`.posts p
		WHERE (f.forum_id = ?) AND (t.topic_id = ?) AND (p.post_id = ?) AND (t.forum_id = f.forum_id)
		AND (p.forum_id = f.forum_id) AND (p.topic_id = t.topic_id)");
	$sql->bind_param("sss", $forum, $topic, $post_id);
} else {
	// No post id, just check forum and topic.
	$sql = $conn->prepare("SELECT f.forum_type, f.forum_name, f.forum_access, t.topic_title FROM forums f, topics t
		WHERE (f.forum_id = ?) AND (t.topic_id = ?) AND (t.forum_id = f.forum_id)");
	$sql->bind_param("ss", $forum, $topic);
}
...
if (isset($submit) && $submit) {
	...
	$sql = $conn->prepare("INSERT INTO `$currentCourseID`.posts (topic_id, forum_id, poster_id, post_time, poster_ip, nom, prenom)
		VALUES (?,?,?,?,?,?,?)");
	$sql->bind_param("sssssss", $topic, $forum, $uid, $time, $poster_ip, $nom, $prenom);
	$sql->execute();
	$result = $sql->get_result();
	$this_post = mysqli_insert_id($conn);
	if ($this_post) {
		$sql = $conn->prepare("INSERT INTO `$currentCourseID`.posts_text (post_id, post_text) VALUES (?, ?)");
		$sql->bind_param("ss", $this_post, $message);
		$sql->execute();
		$result = $sql->get_result():
	}
	$sql = $conn->prepare("UPDATE `$currentCourseID`.topics SET topic_replies = topic_replies+1, topic_last_post_id = ?, topic_time = ? 
		WHERE topic_id = ?");
	$sql->bind_param("sss", $this_post, $time, $topic);
	$sql->execute();
	$result = $sql->get_result():
	$result = db_query($sql, $currentCourseID);
	$sql = $conn->prepare("UPDATE `$currentCourseID`.forums SET forum_posts = forum_posts+1, forum_last_post_id = ?
		WHERE forum_id = ?");
	$sql->bind_param("ss", $this_post, $forum);
	$sql->execute();
	$result = $sql->get_result():
	if (!$result) {
	...
	$tempSql = $conn->prepare("SELECT DISTINCT user_id FROM `$mysqlMainDb`.forum_notify
		WHERE (topic_id = ? OR forum_id = ? OR cat_id = ?)
		AND notify_sent = 1 AND course_id = ?");
	$tempSql->bind_param("sss", $topic, $forum, $category_id);
	$tempSql->execute();
	...
	if (isset($quote) && $quote) {
		$sql = $conn->prepare("SELECT pt.post_text, p.post_time, u.username 
			FROM `$currentCourseID`.posts p, `$currentCourseID`.posts_text pt 
