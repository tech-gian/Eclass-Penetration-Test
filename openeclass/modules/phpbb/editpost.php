...
if ($is_adminOfCourse) { // course admin 
	if (isset($submit) && $submit) {
		$sql = $conn->prepare("SELECT * FROM `$currentCourseID`.posts WHERE post_id = ?");
		$sql->bind_param("s", $post_id);
		if (!$sql->execute()) {
		...
		$tempSql = $conn->prepare("SELECT forum_name FROM `$currentCourseID`.forums WHERE forum_id=?");
		$tempSql->bind_param("s", $forum_id);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$row1 = $tempResult->fetch_assoc();
		$forum_name = $row1['forum_name'];
		$tempSql = $conn->prepare("SELECT topic_title FROM `$currentCourseID`.topics WHERE topic_id=?");
		$tempSql->bind_param("s", $topic_id);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$row1 = $tempResult->fetch_assoc();
      ...
      $message = $purifier->purify($message);
			...
			$sql = $conn->prepare("UPDATE `$currentCourseID`.posts_text SET post_text = ? WHERE post_id = ?");
			$sql->bind_param("ss", $message, $post_id);
			if (!$sql->execute()) {
			...
			if (isset($subject) && (trim($subject) != '')) {
				...
				$sql = $conn->prepare("UPDATE `$currentCourseID`.topics
					SET topic_title = ?, topic_notify = ?
					WHERE topic_id = ?");
				$sql->bind_param("sss", $subject, $notify, $topic_id);
				if (!$sql->execute()) {
			...
			$sql = $conn->prepare("DELETE FROM `$currentCourseID`.posts
				WHERE post_id = ?");
			$sql->bind_param("s", $post_id);
			if (!$sql->execute()){
			...
			$sql = $conn->prepare("DELETE FROM `$currentCourseID`.posts_text
				WHERE post_id = ?");
			$sql->bind_param("s", $post_id);
			if (!$sql->execute()) {
				$tool_content .= $langUnableDeletePost;
				...
				$sql = $conn->prepare("UPDATE `$currentCourseID`.topics
					SET topic_time = ?
					WHERE topic_id = ?");
				$sql->bind_param("ss", $topic_time_fixed, $topic_id);
				if (!$sql->execute()) {
			...
			if (get_total_posts($topic_id, $currentCourseID, "topic") == 0) {
				...
				$sql = $conn->prepare("DELETE FROM `$currentCourseID`.topics
					WHERE topic_id = ?");
		...
		$sql = $conn->prepare("SELECT f.forum_type, f.forum_name, t.topic_title
			FROM `$currentCourseID`.forums f, `$currentCourseID`.topics t
		...
		$sql = $conn->prepare("SELECT p.*, pt.post_text, t.topic_title, t.topic_notify,
			t.topic_title, t.topic_notify
			FROM `$currentCourseID`.posts p, `$currentCourseID`.topics t, `$currentCourseID`.posts_text pt
			WHERE (p.post_id = ?) AND (pt.post_id = p.post_id) AND (p.topic_id = t.topic_id)");
		$sql->bind_param("s", $post_id);
