...
if(isset($forumcatnotify)) { // modify forum category notification
		$tempSql = $conn->prepare("SELECT * FROM `$mysqlMainDb`.forum_notify
			WHERE user_id = ? AND cat_id = ? AND course_id = ?");
		$tempSql->bind_param("sss", $uid, $cat_id, $cours_id);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$row = mysqli_num_rows($tempResult);
		if ($rows > 0) {
			$tempSql = $conn->prepare("UPDATE `$mysqlMainDb`.forum_notify SET notify_sent = ?
				WHERE user_id = ? AND cat_id = ? AND course_id = ?");
			$tempSql->bind_param("ssss", $forumcatnotify, $uid, $cat_id, $cours_id);
			$tempSql->execute();
	} else {
		$tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.forum_notify SET user_id = ?,
			cat_id = ?, notify_sent = 1, course_id =?");
		$tempSql->bind_param("sss", $uid, $cat_id, $cours_id);
		$tempSql->execute();
	}
	...
	$tempSql->bind_param("sss", $uid, $forum_idm, $cours_id);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$rows = mysqli_num_rows($tempResult);
	if ($rows > 0) {
		$tempSql = $conn->prepare("UPDATE `$mysqlMainDb`.forum_notify SET notify_sent = ?
			WHERE user_id = ? AND forum_id = ? AND course_id = ?");
		$tempSql->bind_param("ssss", $forumnotify, $uid, $forum_id, $cours_id);
		$tempSql->execute();
	} else {
		$tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.forum_notify SET user_id = ?,
		forum_id = ?, notify_sent = 1, course_id = ?");
		$tempSql->bind_param("sss", $uid, $forum_id, $cours_id);
		$tempSql->execute();
	}
}
...
if ($total_categories) {
	...
	for($i=0; $i < $total_categories; $i++) {
		...
		$tempSql = $conn->prepare("SELECT notify_sent FROM `$mysqlMainDb`.forum_notify
			WHERE user_id = ? AND cat_id = ? AND course_id = ?");
		$tempSql->bind_param("sss", $uid, $catNum, $cours_id);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$tempRow = $tempResult->fetch_assoc();
		$action_notify = $tempRow['notify_sent'];
		...
		for ($x=0; $x < count($forum_row); $x++) {
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				if ($is_adminOfCourse) { // admin
					$tempSql = $conn->prepare("SELECT id FROM `$currentCourseID`.student_group
						WHERE forumId=? AND tutor=?");
					$tempSql->bind_param("ss", $forum, $uid);
					$tempSql->execute();
					$sqlTutor = $tempSql->get_result();
					$countTutor = mysqli_num_rows($sqlTutor);
					if ($countTutor == 0) {
				...
				$tempSql = $conn->prepare("SELECT notify_sent FROM `$mysqlMainDb`.forum_notify
					WHERE user_id = ? AND forum_id = ? AND course_id = ?");
				$tempSql->bind_param("sss", $uid, $forum, $cours_id);
				$tempSql->execute();
				$tempResult = $tempSql->get_result();
				$tempRow = $tempResult->fetch_assoc();
				$forum_action_notify = $tempRow['notify_sent'];
