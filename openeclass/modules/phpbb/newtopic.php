...
$sql = $conn->prepare("SELECT forum_name, forum_access, forum_type FROM `$currentCourseID`.forums
	WHERE (forum_id = ?)");
...
if (isset($submit) && $submit && validate_csrf_token()) {
	...
	$sql = $conn->prepare("INSERT INTO `$currentCourseID`.topics (topic_title, topic_poster, forum_id,
		topic_time, topic_notify, nom, prenom)
		VALUES (?,?,?,?,1,?,?)");
	$sql->bind_param("ssssss", $subject, $uid, $forum, $time, $nom, $prenom);
	$sql->execute();
	$result = $sql->get_result();
	...
	$sql = $conn->prepare("INSERT INTO `$currentCourseID`.posts (topic_id, forum_id, poster_id, post_time, poster_ip, nom, prenom)
		VALUES (?,?,?,?,?,?,?)");
	$sql->bind_param("sssssss", $topic_id, $forum, $uid, $time, $poster_ip, $nom, $prenom);
	if (!$sql->execute()) {
		...
		if ($post_id) {
			$sql = $conn->prepare("INSERT INTO `$currentCourseID`.posts_text (post_id, post_text)
				VALUES (?,?)");
			$sql->bind_param("ss", $post_id, $message);
			$sql->execute();
			$result = $sql->get_result();
			$sql = $conn->prepare("UPDATE `$currentCourseID`.topics
			SET topic_last_post_id = ?
			WHERE topic_id = ?");
			$sql->bind_param("ss", $post_id, $topic_id);
			$sql->execute();
			$result = $sql->get_result();
		}
	}
	$sql = $conn->prepare("UPDATE `$currentCourseID`.forums
		SET forum_posts = forum_posts+1, forum_topics = forum_topics+1, forum_last_post_id = ?
		WHERE forum_id = ?");
	$sql->bind_param("ss", $post_id, $forum);
	$sql->execute();
	$result = $sql->get_result();
	...
	$tempSql->bind_param("sss", $forum, $category_id, $cours_id);
	$tempSql->execute();
