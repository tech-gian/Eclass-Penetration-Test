...
$sql = $conn->prepare("SELECT f.forum_type, f.forum_name
	FROM `$currentCourseID`.forums f
	WHERE forum_id = ?");
$sql->bind_param("s", $forum);
$sql->execute();
...
$tempSql = $conn->prepare("SELECT COUNT(*) AS num_lines FROM `$currentCourseID`.topics WHERE forum_id = ?");
$tempSql->bind_param("s", $forum);
$tempSql->execute();
$tempResult = $tempSql->get_result();
$topic_count = $tempResult->fetch_assoc();
$total_topics = $topic_count['num_lines'];
...
if(isset($topicnotify)) { // modify topic notification
	...
	$tempSql->bind_param("sss", $uid, $topic_id, $cours_id);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	...
	if ($rows > 0) {
		$tempSql = $conn->prepare("UPDATE `$mysqlMainDb`.forum_notify SET notify_sent = ?
			WHERE user_id = ? AND topic_id = ? AND course_id = ?");
		$tempSql->bind_param("ssss", $topicnotify, $uid, $topic_id, $cours_id);
		$tempSql->execute();
	} else {
		$tempSql = $conn->prepare("INSERT INTO `$mysqlMainDb`.forum_notify SET user_id = ?,
		topic_id = ?, notify_sent = 1, course_id = ?");
		$tempSql->bind_param("sss", $uid, $topic_id, $cours_id);
		$tempSql->execute();
	}
}
...
$sql = $conn->prepare("SELECT t.*, p.post_time, p.nom AS nom1, p.prenom AS prenom1
	FROM `$currentCourseID`.topics t
	LEFT JOIN `$currentCourseID`.posts p ON t.topic_last_post_id = p.post_id
	WHERE t.forum_id = ?
...
if (mysqli_num_rows($result) > 0) { // topics found
	while($myrow = $result->fetch_assoc()) {
		...
		$tempSql = $conn->prepare("SELECT notify_sent FROM `$mysqlMainDb`.forum_notify
			WHERE user_id = ? AND topic_id = ? AND course_id = ?");
		$tempSql->bind_param("sss", $uid, $myrow['topic_id'], $cours_id);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$tempRow = $tempResult->fetch_assoc();
		$topic_action_notify = $tempRow['notify_sent'];
