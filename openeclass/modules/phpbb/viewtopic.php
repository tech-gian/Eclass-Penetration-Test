...
$sql = $conn->prepare("SELECT topic_title, topic_status
	FROM `$currentCourseID`.topics 
	WHERE topic_id = ?");
...
if (isset($_GET['all'])) {
	$sql = $conn->prepare("SELECT p.*, pt.post_text FROM `$currentCourseID`.posts p, posts_text pt 
		WHERE topic_id = ? 
	...
	$sql->bind_param("sss", $topic, $start, $posts_per_page);
	$sql->execute();
} else {
	$sql = $conn->prepare("SELECT p.*, pt.post_text FROM `$currentCourseID`.posts p, `$currentCourseID`.posts_text pt
	WHERE topic_id = ?
	AND p.post_id = pt.post_id
	ORDER BY post_id LIMIT ?");
	$sql->bind_param("ss", $topic, $posts_per_page);
	$sql->execute();
}
$result = $sql->get_result();
if (!$result) {
...
$sql = $conn->prepare("UPDATE `$currentCourseID`.topics SET topic_views = topic_views + 1 WHERE topic_id = ?");
$sql->bind_param("s", $topic);
$sql->execute();
