...
function get_total_topics($forum_id, $thedb) {
	global $langError, $conn;
	$sql = $conn->prepare("SELECT count(*) AS total FROM `$thedb`.topics WHERE forum_id = ?");
	$sql->bind_param("s", $forum_id);
	if(!$sql->execute())
		return($langError);
...
function get_total_posts($id, $thedb, $type) {
   ...
   switch ($type) {
    case 'users':
	  $sql = $conn->prepare("SELECT count(*) AS total FROM `$thedb`.users WHERE (user_id != -1) AND (user_level != -1)");
      break;
    case 'all':
		$sql = $conn->prepare("SELECT count(*) AS total FROM `$thedb`.posts");
      break;
    case 'forum':
		$sql = $conn->prepare("SELECT count(*) AS total FROM `$thedb`.posts WHERE forum_id = ?");
		$sql->bind_param("s", $id);
      break;
    case 'topic':
	  $sql = $conn->prepare("SELECT count(*) AS total FROM `$thedb`.posts WHERE topic_id = ?");
	  $sql->bind_param("s", $id);
      break;
...
function get_last_post($id, $thedb, $type) {
   global $langError, $langNoPosts, $langFrom2, $conn;
   switch($type) {
    case 'time_fix':
	  $sql = $conn->prepare("SELECT p.post_time FROM `$thedb`.posts p WHERE p.topic_id = ? ORDER BY post_time DESC LIMIT 1");
	  $sql->bind_param("s", $id);
      break;
    case 'forum':
	  $sql = $conn->prepare("SELECT p.post_time, p.poster_id FROM `$thedb`.posts p WHERE p.forum_id = ? ORDER BY post_time DESC LIMIT 1");
	  $sql->bind_param("s", $id);
      break;
    case 'topic':
	  $sql = $conn->prepare("SELECT p.post_time FROM `$thedb`.posts p WHERE p.topic_id = ? ORDER BY post_time DESC LIMIT 1");
      ...
      break;
    case 'user':
	  $sql = $conn->prepare("SELECT p.post_time FROM `$thedb`.posts p WHERE p.poster_id = ? LIMIT 1");
	  $sql->bind_param("s", $id);
      break;
   }
   if(!$sql->execute())
...
function does_exists($id, $thedb, $type) {
	...
	switch($type) {
		case 'forum':
			$sql = $conn->prepare("SELECT forum_id FROM `$thedb`.forums WHERE forum_id = ?");
			$sql->bind_param("s", $id);
		break;
		case 'topic':
			$sql = $conn->prepare("SELECT topic_id FROM `$thedb`.topics WHERE topic_id = '?");
			$sql->bind_param("s", $id);
		break;
	}
	if(!$sql->execute())
...
function is_first_post($topic_id, $post_id, $thedb) {
   global $conn;
   $sql = $conn->prepare("SELECT post_id FROM `$thedb`.posts WHERE topic_id = ? ORDER BY post_id LIMIT 1");
	$sql->bind_param("s", $topic_id);
   if(!$sql->execute())
     return(0);
	$r = $sql->get_result();
   if(!$m = $r->fetch_assoc())
...
{
	global $conn;
	$sql = $conn->prepare("SELECT count(*) AS user_count FROM `$db`.forum_access WHERE (user_id = ?) AND (forum_id = ?)");
	$sql->bind_param("ss", $userid, $forumid);
	
	if ($is_posting)
	{
		$sql = $conn->prepare("SELECT count(*) AS user_count FROM `$db`.forum_access WHERE (user_id = ?) AND (forum_id = ?) AND (can_post = 1)");
		$sql->bind_param("ss", $userid, $forumid);
	}
...
function sync($thedb, $id, $type) {
   global $conn;
	switch($type) {
   	case 'forum':
		$sql = $conn->prepare("SELECT max(post_id) AS last_post FROM `$thedb`.posts WHERE forum_id = ?");
		$sql->bind_param("s", $id);
   		if(!$sql->execute())
   		{
		...
		$sql = $conn->prepare("SELECT count(post_id) AS total FROM `$thedb`.posts WHERE forum_id = ?");
		$sql->bind_param("s", $id);
   		if(!$sql->execute())
   		{
		...
		$sql = $conn->prepare("SELECT count(topic_id) AS total FROM `$thedb`.topics WHERE forum_id = ?");
		$sql->bind_param("s", $id);
		if(!$sql->execute())
   		{
		...
		$sql = $conn->prepare("UPDATE `$thedb`.forums
			SET forum_last_post_id = ?, forum_posts = ?, forum_topics = ?
			WHERE forum_id = ?");
		$sql->bind_param("ssss", $last_post, $total_posts, $total_topics, $id);
		if(!$sql->execute())
   		{
   	...
   	case 'topic':
		$sql = $conn->prepare("SELECT max(post_id) AS last_post FROM `$thedb`.posts WHERE topic_id = ?");
		$sql->bind_param("s", $id);
		if(!$sql->execute())
   		{
   			error_die("Could not get post ID");
		...
		$sql = $conn->prepare("SELECT count(post_id) AS total FROM `$thedb`.posts WHERE topic_id = ?");
		$sql->bind_param("s", $id);
		if(!$sql->execute())
   		{
   			error_die("Could not get post count");
   		...
   		$total_posts -= 1;
		$sql = $conn->prepare("UPDATE `$thedb`.topics SET topic_replies = ?, topic_last_post_id = ? WHERE topic_id = ?");
		$sql->bind_param("sss", $total_posts, $last_post, $id);
   		if(!$sql->execute())
   		{
...
function forum_category($id) {
	...
	$tempSql = $conn->prepare("SELECT cat_id FROM `$currentCourseID`.forums WHERE forum_id=?");
...
function category_name($id) {
	...
	global $currentCourseID, $conn;
	$tempSql = $conn->prepare("SELECT cat_title FROM `$currentCourseID`.catagories WHERE cat_id=?");
	$tempSql->bind_param("s", $id);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
