...
$sql = 'SELECT `description`,`course_keywords`, `course_addon`,`faculte`,`lastEdit`,`type`, `visible`, `titulaires`, `fake_code` FROM eclass.`cours` WHERE `code` = "'.$currentCourse.'"';
...
$result = db_query("SELECT MAX(`order`) FROM eclass.course_units WHERE course_id = $cours_id");
list($maxorder) = mysql_fetch_row($result);

// other actions in course unit
if ($is_adminOfCourse) {
        if (isset($_REQUEST['edit_submit'])) {
                $title = autoquote(htmlspecialchars($_REQUEST['unittitle'], ENT_QUOTES));
                $descr = autoquote(htmlspecialchars($_REQUEST['unitdescr'], ENT_QUOTES));
                if (isset($_REQUEST['unit_id'])) { // update course unit
                        $unit_id = intval($_REQUEST['unit_id']);
                        $result = db_query("UPDATE eclass.course_units SET
                                                   title = $title,
                                                   comments = $descr
		        ...
		        $main_content .= "\n      <p class='success_small'>$langCourseUnitModified</p>";
                        ...
                        $order = $maxorder + 1;
                        db_query("INSERT INTO eclass.course_units SET
		...
		db_query("DELETE FROM eclass.course_units WHERE id = '$id'");
		db_query("DELETE FROM eclass.unit_resources WHERE unit_id = '$id'");
		...
		$sql = db_query("SELECT `visibility` FROM eclass.course_units WHERE id='$id'");
		list($vis) = mysql_fetch_row($sql);
		$newvis = ($vis == 'v')? 'i': 'v';
		db_query("UPDATE eclass.course_units SET visibility = '$newvis' WHERE id = $id AND course_id = $cours_id");
...
if ($is_adminOfCourse) {
	$cunits_content .= ": <a href='{$urlServer}modules/units/info.php'>$langAddUnit</a>&nbsp;<img src='../../template/classic/img/add.gif' width='15' height='15' title='$langAddUnit' alt='$langAddUnit' />";
        ...
        $cunits_content .= "</td>\n      </tr>\n      </thead>\n      </table>\n";
if ($is_adminOfCourse) {
        list($last_id) = mysql_fetch_row(db_query("SELECT id FROM eclass.course_units
                                                   WHERE course_id = $cours_id
                                                   ORDER BY `order` DESC LIMIT 1"));
	$query = "SELECT id, title, comments, visibility
		  FROM eclass.course_units WHERE course_id = $cours_id
                  ORDER BY `order`";
} else {
	$query = "SELECT id, title, comments, visibility
		  FROM eclass.course_units WHERE course_id = $cours_id AND visibility='v'
                  ORDER BY `order`";
}
$sql = db_query($query);
$first = true;
