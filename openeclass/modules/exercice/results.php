...
while($row=mysql_fetch_array($result)) {
	$sid = $row['uid'];
	$tempSql = $conn->prepare("SELECT nom,prenom,am FROM user WHERE user_id=?");
	...
	mysql_select_db($currentCourseID);
	mysqli_select_db($conn, $currentCourseID);
	$sql2 = $conn->prepare("SELECT DATE_FORMAT(RecordStartDate, '%Y-%m-%d / %H:%i') AS RecordStartDate, RecordEndDate, TIME_TO_SEC(TIMEDIFF(RecordEndDate,RecordStartDate)) AS TimeDuration, TotalScore, TotalWeighting 
		FROM `exercise_user_record` WHERE uid=? AND eid=?");
	$sql2->bind_param("ss", $sid, $exerciseId);
	$sql2->execute();
	$result2 = $sql2->get_result();
	if (mysqli_num_rows($result2) > 0) { // if users found
