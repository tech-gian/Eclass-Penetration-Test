...
if($is_adminOfCourse) {
	...
	$sql = $conn->prepare("SELECT id, titre, description, type, active FROM `$currentCourseID`.`$TBL_EXERCICES` ORDER BY id LIMIT $from, $limitExPage");
	$sql->execute();
	$result = $sql->get_result();
	$tempSql = $conn->prepare("SELECT count(*) as num_lines FROM `$currentCourseID`.`$TBL_EXERCICES`");
	$tempSql->execute();
	$qnum = $tempSql->get_result();
} else {
        // only for students
	$sql = $conn->prepare("SELECT id, titre, description, type, StartDate, EndDate, TimeConstrain, AttemptsAllowed 
		FROM `$currentCourseID`.`$TBL_EXERCICES` WHERE active='1' ORDER BY id LIMIT $from, $limitExPage");
	$sql->execute();
	$result = $sql->get_result();
	$tempSql = $conn->prepare("SELECT count(*) as num_lines FROM `$currentCourseID`.`$TBL_EXERCICES` WHERE active = 1");
	$tempSql->execute();
	$qnum = $tempSql->get_result();
}

$tempRow = $qnum->fetch_assoc();
$num_of_ex = $tempRow['num_lines'];
$nbrExercises = mysqli_num_rows($result);
...
while($row = $result->fetch_assoc()) {
	...
	// prof only
		...
		$eid = $row['id'];
		$tempSql = $conn->prepare("SELECT COUNT(*) as num_lines FROM `$currentCourseID`.exercise_user_record 
			WHERE eid=?");
		$tempSql->bind_param("s", $eid);
		$tempSql->execute();
		$tempResult = $tempSql->get_result();
		$NumOfResults = $tempResult->fetch_assoc();

	if ($NumOfResults['num_lines']) {
...
// how many attempts we have.
	$tempSql = $conn->prepare("SELECT COUNT(*) as num_lines FROM `$currentCourseID`.exercise_user_record 
		WHERE eid=? AND uid=?");
	$tempSql->bind_param("ss", $row['id'], $uid);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$CurrentAttempt = $tempResult->fetch_assoc();
	...
	if ($row['AttemptsAllowed'] > 0) {
		   $tool_content .= "<td align='center'><small>$CurrentAttempt[num_lines]/$row[AttemptsAllowed]</small></td>";
	} else {
		 $tool_content .= "<td align='center'><small> - </small></td>";
	}
	  $tool_content .= "</tr>";
