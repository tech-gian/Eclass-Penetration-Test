...
class Question
{
	...
	function read($id)
	{
		global $TBL_QUESTIONS, $TBL_EXERCICE_QUESTION, $currentCourseID, $conn;
		
		mysql_select_db($currentCourseID);
		mysqli_select_db($conn, $currentCourseID);
		$sql = $conn->prepare("SELECT question,description,ponderation,q_position,type FROM `$TBL_QUESTIONS` WHERE id=?");
		$sql->bind_param("s", $id);
		$sql->execute() or die("Error : SELECT in file ".__FILE__." at line ".__LINE__);
		$result = $sql->get_result();

		// if the question has been found
		if($object=$result->fetch_assoc())
		{
			$this->id=$id;
			...
			$sql = $conn->prepare("SELECT exercice_id FROM `$TBL_EXERCICE_QUESTION` WHERE question_id=?");
			$sql->bind_param("s", $id);
			$sql->execute() or die("Error : SELECT in file ".__FILE__." at line ".__LINE__);
			$result = $sql->get_result();
	...
	{
		global $TBL_REPONSES, $currentCourseID, $conn;
		...
		if($type != $this->type)
		{
			...
			if(!in_array($this->type,array(UNIQUE_ANSWER,MULTIPLE_ANSWER)) || !in_array($type,array(UNIQUE_ANSWER,MULTIPLE_ANSWER)))
			{
				...
				$sql = $conn->prepare("DELETE FROM `$TBL_REPONSES` WHERE question_id=?");
				$sql->bind_param("s", $this->id);
				$sql->execute() or die("Error : DELETE in file ".__FILE__." at line ".__LINE__);
			}
	...
	{
		global $TBL_QUESTIONS, $currentCourseID, $conn;
		...
		if($id)
		{
			$sql = $conn->prepare("UPDATE `$TBL_QUESTIONS` SET question=?,description=?,ponderation=?,q_position=?,type=? WHERE id=?");
			$sql->bind_param("ssssss", $question, $description, $weighting, $position, $type, $id);
			$sql->execute() or die("Error : UPDATE in file ".__FILE__." at line ".__LINE__);
		}
		// creates a new question
		else
		{
			$sql = $conn->prepare("INSERT INTO `$TBL_QUESTIONS`(question,description,ponderation,q_position,type) VALUES(?,?,?,?,?)");
			$sql->bind_param("sssss", $question, $description, $weighting, $position, $type);
			$sql->execute() or die("Error : INSERT in file ".__FILE__." at line ".__LINE__);
	...
	{
		global $TBL_EXERCICE_QUESTION, $conn;
		...
		if(!in_array($exerciseId,$this->exerciseList))
		{
			...
			$sql = $conn->prepare("INSERT INTO `$TBL_EXERCICE_QUESTION`(question_id,exercice_id) VALUES(?, ?)");
			$sql->bind_param("ss", $id, $exerciseId);
			$sql->execute() or die("Error : INSERT in file ".__FILE__." at line ".__LINE__);
		}
	}
	...
	function removeFromList($exerciseId)
	{
		...
		else
		{
			...
			$sql = $conn->prepare("DELETE FROM `$TBL_EXERCICE_QUESTION` WHERE question_id=? AND exercice_id=?");
			$sql->bind_param("ss", $id, $exerciseId);
			$sql->execute();
			return true;
		}
	...
	{
		global $TBL_EXERCICE_QUESTION, $TBL_QUESTIONS, $TBL_REPONSES, $conn;
	...
	// if the question must be removed from all exercises
		...
		if(!$deleteFromEx)
		{
			$sql = $conn->prepare("DELETE FROM `$TBL_EXERCICE_QUESTION` WHERE question_id=?");
			$sql->bind_param("s", $id);
			$sql->execute();

			$sql = $conn->prepare("DELETE FROM `$TBL_QUESTIONS` WHERE id=?");
			$sql->bind_param("s", $id);
			$sql->execute();
	...
	function duplicate()
	{
		global $TBL_QUESTIONS, $picturePath, $currentCourseID, $conn;
		...
		$sql = $conn->prepare("INSERT INTO `$currentCourseID`.`$TBL_QUESTIONS`(question,description,ponderation,q_position,type) 
			VALUES(?,?,?,?,?)");
		$sql->bind_param("sssss", $questionId, $description, $weighting, $position, $type);
		$sql->execute();
