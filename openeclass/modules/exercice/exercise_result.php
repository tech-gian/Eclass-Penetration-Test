...
$sql = $conn->prepare("SELECT RecordStartDate FROM `exercise_user_record` WHERE eid=? AND uid=?");
$sql->bind_param("ss", $eid, $uid);
$sql->execute();
$result = $sql->get_result();
$attempt = count($result);

$sql = $conn->prepare("SELECT MAX(eurid) as max_eurid FROM `exercise_user_record` WHERE eid=? AND uid=?");
$sql->bind_param("ss", $eid, $uid);
$sql->execute();
$result = $sql->get_result();
...
$sql = $conn->prepare("UPDATE `$currentCourseID`.exercise_user_record SET TotalScore=?, TotalWeighting=?,
	attempt=? WHERE eurid=?");
$sql->bind_param("ssss", $totalScore, $totalWeighting, $attempt, $eurid);
$sql->execute();
