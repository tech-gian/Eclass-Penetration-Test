...
if (isset($exerciseId)) {
	// security check 
	$tempSql = $conn->prepare("SELECT `$currentCourseID`.active FROM `$TBL_EXERCICES` 
		WHERE id=?");
	$tempSql->bind_param("s", $exerciseId);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$active = $tempResult->fetch_assoc();
...
if (isset($formSent)) {
	$tempSql = $conn->prepare("SELECT COUNT(*) AS num_lines FROM `$currentCourseID`.exercise_user_record 
		WHERE eid=? AND uid=?");
	$tempSql->bind_param("ss", $eid_temp, $uid);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$CurrentAttempt = $tempResult->fetch_assoc();
	++$CurrentAttempt['num_lines'];
	if (($exerciseAllowedAttemtps == 0) or ($CurrentAttempt['num_lines'] <= $exerciseAllowedAttemtps)) { // if it is allowed
		...
		if (($exerciseType == 1) or (($exerciseType == 2) and ($nbrQuestions == $questionNum))) { // record
			...
			mysqli_select_db($currentCourseID);
			$sql = $conn->prepare("INSERT INTO `$currentCourseID`.exercise_user_record(eid, uid, RecordStartDate, RecordEndDate, attempt)
				VALUES (?, ?, ?, ?, 1)");
			$sql->bind_param("ssss", $eid_temp, $uid, $RecordStartDate, $RecordEndDate);
			$sql->execute();
