...
if(isset($submitQuestion)) {
	$questionName = htmlspecialchars(trim($questionName));
	$questionDescription = htmlspecialchars(trim($questionDescription));
...
if(isset($newQuestion) || isset($modifyQuestion)) {
	...
	@$tool_content .= "<form enctype='multipart/form-data' method='post' action='" . htmlspecialchars($_SERVER['PHP_SELF']) . "?modifyQuestion=$modifyQuestion&newQuestion=$newQuestion'>
