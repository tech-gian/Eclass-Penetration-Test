...
if(isset($_REQUEST['creation']) && $is_adminOfCourse) {
	// Create a hidden category for group forums
        ...
        if ($req and mysql_num_rows($req) > 0) {
			...
			$langCatagoryGroup = q($langCatagoryGroup);
	...
	for ($i = 1; $i <= $group_quantity; $i++) {
		...
		$langForumGroup = q($$langForumGroup);
		$cat_id = q($cat_id);
...
if ($is_adminOfCourse) {
	...
	while ($group = mysql_fetch_array($groupSelect)) {
		$group["name"] = q($group["name"]);
		$group["tutor"] = q($group["tutor"]);
		$group["maxStudent"] = q($group["maxStudent"]);
