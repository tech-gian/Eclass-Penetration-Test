...
function process_extracted_file($p_event, &$p_header) {
        ...
        $filename = q(array_pop($path_components));
        ...
        if ($p_header['folder']) {
                ...
                db_query("INSERT INTO document SET
                                 path = '$path',
                                 filename = " . quote(q($filename)) .",
                                 visibility = 'v',
                                 comment = " . quote(q($file_comment)) . ",
                                 category = " . quote(q($file_category)) . ",
                                 title = " . quote(q($file_title)) . ",
                                 creator = " . quote(q($file_creator)) . ",
                                 date = " . quote(q($file_date)) . ",
                                 date_modified = " . quote(q($file_date)) . ",
                                 subject = " . quote(q($file_subject)) . ",
                                 description = " . quote(q($file_description)) . ",
                                 author = " . quote(q($file_author)) . ",
                                 format = '$format',
                                 language = " . quote(q($file_language)) . ",
                                 copyrighted = " . quote(q($file_copyrighted)));
...
function make_path($path, $path_components)
{
        ...
        foreach ($path_components as $component) {
                ...
                if (mysql_num_rows($q) > 0) {
                        ...
                        db_query("INSERT INTO document SET
    				  path='$path',
                                  filename=" . quote(q($component)) . ",
    				  visibility='v',
                                  creator=" . quote(q($prenom)." ".q($nom)) . ",
                                  date=NOW(),
                                  date_modified=NOW(),
                                  format='.dir'");
...
{       // teacher only
	...
	***********************************************************************/
	...
	$dialogBox = '';
	if (is_uploaded_file(@$userFile) && validate_csrf_token()) {
		...
		if ($diskUsed + @$_FILES['userFile']['size'] > $diskQuotaDocument) {
			$dialogBox .= "<p class='caution_small'>$langNoSpace</p>";
                        ...
                        {
                        $fileName = q(trim ($_FILES['userFile']['name']));
                        ...
                        $row = mysql_fetch_array($result);
                                ...
                                } else //to arxeio den vrethike sth vash ara mporoume na proxwrhsoume me to upload
                                {
                                        ...
                                        path	=	'".mysql_real_escape_string(q($uploadPath2))."',
                                        filename =	'".mysql_real_escape_string(q($fileName))."',
                                        visibility =	'v',
                                        comment	=	'".mysql_real_escape_string(q($file_comment))."',
                                        category =	'".mysql_real_escape_string(q($file_category))."',
                                        title =	'".mysql_real_escape_string(q($file_title))."',
                                        creator	=	'".mysql_real_escape_string(q($file_creator))."',
                                        date	= '".mysql_real_escape_string(q($file_date))."',
                                        date_modified	=	'".mysql_real_escape_string(q($file_date))."',
                                        subject	=	'".mysql_real_escape_string(q($file_subject))."',
                                        description =	'".mysql_real_escape_string(q($file_description))."',
                                        author	=	'".mysql_real_escape_string(q($file_author))."',
                                        format	=	'".mysql_real_escape_string(q($file_format))."',
                                        language =	'".mysql_real_escape_string(q($file_language))."',
                                        copyrighted	=	'".mysql_real_escape_string(q($file_copyrighted))."'";
	...
	******************************************/
	...
	if (isset($renameTo2) && validate_csrf_token()) {
		$query =  "UPDATE $dbTable SET filename=" .
                        quote(canonicalize_whitespace(q($renameTo2))) .
	...
	if (isset($newDirPath) && validate_csrf_token()) {
                $newDirName = canonicalize_whitespace(q($newDirName));
                if (!empty($newDirName)) {
	...
	if (isset($edit_metadata) && validate_csrf_token())
	{
		...
		//den yparxei eggrafh sth vash gia to sygkekrimeno arxeio opote dhmiourghse thn eggrafh
		{
			...
			$query =  "INSERT INTO ".$dbTable." SET
    			path=\"".$commentPath."\",
    			filename=\"".q($file_filename)."\",
    			visibility=\"v\",
				comment=\"".mysql_real_escape_string(q($file_comment))."\",
				category=\"".mysql_real_escape_string(q($file_category))."\",
				title=\"".mysql_real_escape_string(q($file_title))."\",
				creator=\"".q($prenom)." ".q($nom)."\",
	...
	if (isset($comment))
	{
		...
		$fileName = q(my_basename($comment));
		if (empty($oldFilename)) $oldFilename = $fileName;
		$dialogBox .="	<form method=\"post\" action=\"" . htmlspecialchars($_SERVER['PHP_SELF']) . "?edit_metadata\">
