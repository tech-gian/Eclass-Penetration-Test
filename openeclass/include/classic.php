...
$tool_content = "";
$status = array();
$tempSql = $conn->prepare("SELECT cours.cours_id cours_id, cours.code code, cours.fake_code fake_code,
                cours.intitule title, cours.titulaires profs, cours_user.statut statut
                FROM eclass.cours JOIN eclass.cours_user ON cours.cours_id = cours_user.cours_id
                WHERE cours_user.user_id = ?
                ORDER BY statut, cours.intitule, cours.titulaires");
$tempSql->bind_param("s", $uid);
$tempSql->execute();
$result2 = $tempSql->get_result();
if ($result2 and mysqli_num_rows($result2) > 0) {
	$k = 0;
        $this_statut = 0;
	// display courses
	while ($mycours = $result2->fetch_assoc()) {
...
if (count($status) > 0) {
        ...
        foreach ($status as $code => $code_statut) {
                ...
                $tempSql = $conn->prepare("SELECT contenu, temps, title
                        FROM eclass.annonces, `$code`.accueil
                        WHERE cours_id = ?
                        AND temps > DATE_SUB(?, INTERVAL 10 DAY)
                        AND `$code`.accueil.visible = 1
                        AND `$code`.accueil.id = 7
                        ORDER BY temps DESC");
                $tempSql->bind_param("ss", $cid, $logindate);
                $tempSql->execute();
