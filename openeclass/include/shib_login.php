...
$sqlLogin = $conn->prepare("SELECT user_id, nom, username, password, prenom, statut, email, iduser is_admin, perso, lang
	FROM user LEFT JOIN admin
	ON user.user_id = admin.iduser
	WHERE username=?");
$sqlLogin->bind_param("s", $shib_uname);
$sqlLogin->execute();
$r = $sqlLogin->get_result();
if (mysqli_num_rows($r) > 0) { // if shibboleth user found 
	while ($myrow = $r->fetch_assoc()) {
		// update user information
		$tempSql = $conn->prepare("UPDATE user SET nom = ?, prenom = ?, email = ? 
			WHERE username = ?");
		$tempSql->bind_param("ssss", $shib_nom, $shib_prenom, $shib_email, $shib_uname);
		$tempSql->execute();

		$r2 = $sqlLogin->get_result();
		while ($myrow2 = $r2->fetch_assoc()) {
			$uid = $myrow2["user_id"];
	...
	$tempSql = $conn->prepare("INSERT INTO user SET nom=?, prenom=?, password='shibboleth', 
		username=?,email=?, statut=5, lang='el'");
	$tempSql->bind_param("ssss", $shib_nom, $shib_prenom, $shib_uname, $shib_email);
	$tempSql->execute();
	$uid = $tempSql->insert_id;
	$userPerso = 'yes';
	$nom = $shib_nom;
	$prenom = $shib_prenom;
