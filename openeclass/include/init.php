...
// Connect to database
// TODO: Remove when everything gets replaced
...
if (isset($require_current_course) and $require_current_course) {
	if (!isset($_SESSION['dbname'])) {
		...
		$tempSql = $conn->prepare("SELECT cours_id, code, fake_code, intitule, faculte,
			titulaires, languageCourse,
			departmentUrlName, departmentUrl, visible
			FROM cours WHERE cours.code=?");
		$tempSql->bind_param("s", $dbname);
		$tempSql->execute();
		...
		while ($theCourse = $result->fetch_assoc()) {
		...
		if (isset($uid)) {
					...
					$tempSql = $conn->prepare("SELECT statut FROM cours_user
						WHERE user_id = ? AND
						cours_id = ?");
					$tempSql->bind_param("ss", $uid, $cours_id);
					$tempSql->execute();
					$res2 = $tempSql->get_result();
        			if ($res2 and mysqli_num_rows($res2) > 0) {
						$tempRow = $res2->fetch_assoc();
						$statut = $tempRow['statut'];
		        	}
                        }
		}
...
if(isset($currentCourse) && file_exists($module_ini_dir = getcwd() . "/module.ini.php") && !$is_adminOfCourse && @$ignore_module_ini != true) {
	...
	if (!check_guest()) {
		if (isset($_SESSION['uid']) and $_SESSION['uid']) {
			$tempSql = $conn->prepare("select `id` from `$currentCourse`.accueil
				where visible=1
				ORDER BY rubrique");
			$tempSql->execute();
			$result = $tempSql->get_result();
	...
	$publicModules = array();
	while ($moduleIDs = $result->fetch_assoc()) {
