...
function getToolsArray($cat) {
	global $currentCourse, $currentCourseID, $conn;
		...
		case 'Public':
			if (!check_guest()) {
				if (isset($_SESSION['uid']) and $_SESSION['uid']) {
					$tempSql = $conn->prepare("SELECT * FROM `$currentCourseID`.accueil
						WHERE visible=1
						ORDER BY rubrique");
					$tempSql->execute();
 ...
 */
function loggedInMenu(){
	global $webDir, $language, $uid, $is_admin, $urlServer, $mysqlMainDb, $conn;
	...
	$tempSql = $conn->prepare("SELECT statut FROM eclass.user WHERE user_id = ?");
	$tempSql->bind_param("s", $uid);
	$tempSql->execute();
	$res2 = $tempSql->get_result();

	if ($row = $res2->fetch_assoc()) $statut = $row['statut'];
 ...
 */
function lessonToolsMenu(){
	global $is_admin, $is_adminOfCourse, $uid, $mysqlMainDb, $conn;
	...
	while ($toolsRow = $result->fetch_assoc()) {
