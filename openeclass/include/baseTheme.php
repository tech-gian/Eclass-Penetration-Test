...
if ($is_adminOfCourse and isset($currentCourseID)) {
	if (isset($_GET['hide']) and $_GET['hide'] == 0) {
		$tempSql = $conn->prepare("UPDATE accueil SET visible = 0 WHERE id=?");
		$tempSql->bind_param("s", $eclass_module_id);
		$tempSql->execute();
	} else if (isset($_GET['hide']) and $_GET['hide'] == 1) {
		$tempSql = $conn->prepare("UPDATE accueil SET visible = 1 WHERE id=?");
		$tempSql->bind_param("s", $eclass_module_id);
		$tempSql->execute();
	}
}
