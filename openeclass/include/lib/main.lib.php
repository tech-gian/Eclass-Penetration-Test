...
function uid_to_username($uid)
{
	...
	$tempSql = $conn->prepare("SELECT username FROM `$mysqlMainDb`.user WHERE user_id = ?");
...
function uid_to_name($uid)
{
	...
	$tempSql = $conn->prepare("SELECT CONCAT(nom, ' ', prenom) AS fullname
		FROM `$mysqlMainDb`.user WHERE user_id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($uid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$r = $tempResult->fetch_assoc();
	if ($r) {
...
function uid_to_firstname($uid)
{
	...
	$tempSql = $conn->prepare("SELECT prenom
		FROM `$mysqlMainDb`.user WHERE user_id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($uid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
...
function uid_to_surname($uid)
{
	...
	$tempSql = $conn->prepare("SELECT nom
		FROM `$mysqlMainDb`.user WHERE user_id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($uid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
...
function uid_to_email($uid)
{
	...
	$tempSql = $conn->prepare("SELECT email
		FROM `$mysqlMainDb`.user WHERE user_id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($uid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
...
function uid_to_am($uid)
{
	...
	$tempSql = $conn->prepare("SELECT am from `$mysqlMainDb`.user
		WHERE user_id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($uid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
...
function user_group($uid, $required = TRUE)
{
	...
	$tempSql = $conn->prepare("SELECT team FROM user_group WHERE user = ?");
	$tempSql->bind_param("s", $uid);
	$tempSql->execute();
	$res = $tempSql->get_result();
	if ($res) {
		$secret = $res->fetch_assoc();
		return $secret['team'];
	} else {
		if ($required) {
...
{
	global $currentCourseID, $conn;
	$tempSql = $conn->prepare("SELECT name FROM student_group
		WHERE id = ?");
	$tempSql->bind_param("s", mysql_real_escape_string($gid));
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$r = $tempResult->fetch_assoc();
	if ($r) {
...
function group_secret($gid)
{
	...
	$tempSql = $conn->prepare("SELECT secretDirectory FROM student_group WHERE id = ?");
	$tempSql->bind_param("s", $gid);
	$tempSql->execute();
	$res = $tempSql->get_result();
	if ($res) {
		$secret = $res->fetch_assoc();
		return $secret['secretDirectory'];
	} else {
...
function check_admin() {
	...
	if (isset($uid)) {
		$tempSql = $conn->prepare("SELECT * FROM admin WHERE idUser= ?");
		$tempSql->bind_param("s", $uid);
		$tempSql->execute();
		$res = $tempSql->get_result();
	}
	if (!isset($uid) or !$res or mysqli_num_rows($res) == 0) {
		return false;
	} else return true;
}
...
function check_guest() {
	global $mysqlMainDb, $uid, $conn;
	if (isset($uid)) {
		$tempSql = $conn->prepare("SELECT statut FROM `$mysqlMainDb`.user WHERE user_id = ?");
		$tempSql->bind_param("s", $uid);
		$tempSql->execute();
		$res = $tempSql->get_result();
		$g = $res->fetch_assoc();
...
function check_prof()
{
	global $mysqlMainDb, $uid, $require_current_course, $is_adminOfCourse, $conn;
	if (isset($uid)) {
...
function user_exists($login) {
  ...
  $tempSql = $conn->prepare("SELECT username FROM `$mysqlMainDb`.user WHERE username = ?");
  $tempSql->bind_param("s", mysql_real_escape_string($login));
  $tempSql->execute();
  $username_check = $tempSql->get_result();
...
function find_faculty_by_id($id) {
	...
	$tempSql = $conn->prepare("SELECT name FROM faculte WHERE id = ?");
	$tempSql->bind_param("s", $id);
	$tempSql->execute();
	$req = $tempSql->get_result();

	if ($req and mysqli_num_rows($req)) {
		$fac = $req->fetch_assoc();
		return $fac['name'];
	} else {
		$tempSql = $conn->prepare("SELECT name FROM faculte WHERE name = ?");
		$tempSql->bind_param("s", addslashes($id));
		$tempSql->execute();
		$req - $tempSql->get_result();
...
function new_code($fac) {
	...
	$tempSql = $conn->prepare("SELECT code, generator
		FROM faculte WHERE id = ?");
	$tempSql->bind_param("s", $fac);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$gencode = $tempResult->fetch_assoc();
	do {
		$code = $gencode['code'].$gencode['generator'];
		$gencode['generator'] += 1;
		$generator = $gencode['generator'];
		db_query("UPDATE $mysqlMainDb.faculte SET generator = '$generator'
			WHERE id = '$fac'");
	} while (mysql_select_db($code) and mysqli_select_db($conn, $code));
	mysql_select_db($mysqlMainDb);
	mysqli_select_db($conn, $mysqlMainDb);
...
function last_login($uid)
{
		...
		$tempSql = $conn->prepare("SELECT DATE_FORMAT(MAX(`when`), '%Y-%m-%d') as last_login FROM `$mysqlMainDb`.loginout
			WHERE id_user = ? AND action = 'LOGIN'");
		$tempSql->bind_param("s", $uid);
		$tempSql->execute();
		$q = $tempSql->get_result();
		$row = $q->fetch_assoc();
...
function check_new_announce() {
		...
		$tempSql = $conn->prepare("SELECT * FROM annonces, cours_user
			WHERE annonces.cours_id = cours_user.cours_id AND
			cours_user.user_id = ? AND
			annonces.temps >= ?
			ORDER BY temps DESC LIMIT 1");
		$tempSql->bind_param("ss", $uid, $lastlogin);
		$tempSql->execute();
		$q = $tempSql->get_result();
...
{
	global $langRoot, $userGroupId, $conn;
	...
	foreach (explode('/', $path) as $component) {
		if (empty($component)) {
			...
			$tempSql = $conn->prepare("SELECT filename FROM $dbTable
				WHERE path LIKE CONCAT('%', ?)");
			$tempSql->bind_param("s", $component);
			$tempResult = $tempSql->execute();
...
function display_activation_link($module_id) {
	...
	$tempSql = $conn->prepare("SELECT lien FROM `$currentCourseID`.accueil
		WHERE id = ?");
	$tempSql->bind_param("s", $module_id);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
	$v = $tempResult->fetch_assoc();
...
function visible_module($module_id) {
	...
	$tempSql = $conn->prepare("SELECT visible FROM `$currentCourseID`.accueil
	WHERE id = ?");
	$tempSql->bind_param("s", $module_id);
	$tempSql->execute();
	$tempResult = $tempSql->get_result();
...
function move_order($table, $id_field, $id, $order_field, $direction, $condition = '')
{
	global $conn;
		...
		$tempSql = $conn->prepare("SELECT `$order_field` FROM `$table`
			WHERE `$id_field` = ?");
		$tempSql->bind_param("s", $id);
		$tempSql->execute();
		$sql = $tempSql->get_result();
        if (!$sql or mysqli_num_rows($sql) == 0) {
                return false;
        }
        list($current) = $sql->fetch_assoc();

		$tempSql = $conn->prepare("SELECT `$id_field`, `$order_field` FROM `$table`
			WHERE `order` $op '$current' ?
			ORDER BY ? $desc LIMIT 1");
		$tempSql->bind_param("ss", $condition, $order_field);
		$tempSql->execute();
		$sql = $tempSql->get_result();

        if ($sql and mysqli_num_rows($sql) > 0) {
                list($next_id, $next) = $sql->fetch_assoc();
				$tempSql = $conn->prepare("UPDATE ? SET ? = $next
					WHERE ? = ?");
				$tempSql->bind_param("ssss", $table, $order_field, $id_field, $id);
				$tempSql->execute();
				
				$tempSql = $conn->prepare("UPDATE ? SET ? = $current
					WHERE ? = ?");
				$tempSql->bind_param("ssss", $table, $order_field, $id_field, $next_id);
				$tempSql->execute();
...
{
        global $navigation, $cours_id, $is_adminOfCourse, $mysqlMainDb, $conn;
		...
		if (isset($_GET['unit'])) {
			...
			$unit_id = intval($_SESSION['unit']);
				...
				$tempSql = $conn->prepare("SELECT title FROM `$mysqlMainDb`.course_units
					WHERE id=? AND course_id=? " .
					$visibility_check);
				$tempSql->bind_param("ss", $unit_id, $cours_id);
				$tempSql->execute();
				$q = $tempSql->get_result();
                if ($q and mysqli_num_rows($q) > 0) {
                        list($unit_name) = $q->fetch_assoc();
...
{
        global $mysqlMainDb, $conn;
		$tempSql = $conn->prepare("SELECT intitule FROM `$mysqlMainDb`.cours WHERE code=?");
		if ($conn->error) {
			$test = $conn->error;
			echo "<script>console.log('$test');</script>";
		}
		$tempSql->bind_param("s", $code);
		$tempSql->execute();
		$r = $tempSql->get_result();
...
{
		global $mysqlMainDb, $conn;
		$tempSql = $conn->prepare("SELECT cours_id FROM `$mysqlMainDb`.cours WHERE code=?");
		$tempSql->bind_param("s", $code);
		$tempSql->execute();
		$r = $tempSql->get_result();
        if ($r and mysqli_num_rows($r) > 0) {
...
{
		global $conn;
		$tempSql = $conn->prepare("SELECT value FROM config WHERE `key` = ?");
		$tempSql->bind_param("s", $key);
		$tempSql->execute();
		$r = $tempSql->get_result();
        if ($r and mysqli_num_rows($r) > 0) {
                $row = $r->fetch_assoc();
