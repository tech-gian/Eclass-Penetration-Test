...
function commentBox($type, $mode)
{
    ...
    global $langDefaultModuleAddedComment, $langDelete, $conn;
    ...
    switch ( $type )
    {
        case MODULE_ :
            ...
            $where_cond = "`module_id` = ?";  // use backticks ( ` ) for col names and simple quote ( ' ) for string
        ...
        case LEARNINGPATH_ :
            ...
            $where_cond = '`learnPath_id` = ?';  // use backticks ( ` ) for col names and simple quote ( ' ) for string
            $params = array($_SESSION['path_id']);
            break;
        case LEARNINGPATHMODULE_ :
            ...
            $where_cond = "`learnPath_id` = ? AND `module_id` = ?";
            $params = array($_SESSION['path_id'], $_SESSION['lp_module_id']);
            break;
    }
    ...
    if ( ( $mode == UPDATE_ || $mode == UPDATENOTSHOWN_ )  && $is_adminOfCourse )
    {
        ...
        {
            $_POST['insertCommentBox'] = q($_POST['insertCommentBox']);
            $sql = $conn->prepare("UPDATE `" . $tbl_name . "` SET `" . $col_name . "` = ? WHERE " . $where_cond);
            if (count($params) == 1) {
                $sql->bind_param("ss", addslashes($_POST['insertCommentBox']), $params[0]);
            }
            else {
                $sql->bind_param("sss", addslashes($_POST['insertCommentBox']), $params[0], $params[1]);   
            }
            $sql->execute();
        ...
        else // display form
        {
            ...
            $sql = $conn->prepare("SELECT `".$col_name."`
                FROM `" . $tbl_name . "`
                WHERE " . $where_cond);
            if (count($params) == 1) {
                $sql->bind_param("ss", addslashes($_POST['insertCommentBox']), $params[0]);
            ...
            $sql->execute();
            $tempResult = $sql->get_result();
            $oldComment = $tempResult->fetch_assoc();
    ...
    {
        $sql = $conn->prepare("UPDATE `" . $tbl_name . "`
            SET `" . $col_name . "` = ''
            WHERE " . $where_cond);
        if (count($params) == 1) {
            $sql->bind_param("ss", addslashes($_POST['insertCommentBox']), $params[0]);
...
function nameBox($type, $mode, $formlabel = FALSE)
{
    ...
    global $langModify, $langOk, $langErrorNameAlreadyExists, $conn;
    ...
    switch ( $type )
    {
        case MODULE_ :
            $col_name = 'name';
            $tbl_name = $tbl_lp_module;
            $where_cond = '`module_id` = ?';
            $param = $_SESSION['lp_module_id'];
            break;
        case LEARNINGPATH_ :
            $col_name = 'name';
            $tbl_name = $tbl_lp_learnPath;
            $where_cond = '`learnPath_id` = ';
            $param = $_SESSION['path_id'];
            break;
    }
    ...
    if ( $mode == UPDATE_ && $is_adminOfCourse)
    {
        ...
        {
            $sql = $conn->prepare("SELECT COUNT(`" . $col_name . "`)
                FROM `" . $tbl_name . "`
                WHERE `" . $col_name . "` = ? AND !(" . $where_cond . ")");
            $sql->bind_param("ss", addslashes($_POST['newName']), $param);
            $sql->execute();
            $tempResult = $sql->get_result();
            $num = $tempResult->fetch_assoc();

            if ($num == 0)  // name doesn't already exists
            {
                $_POST['newName'] = q($_POST['newName']);
                $sql = $conn->prepare("UPDATE `" . $tbl_name . "`
                    SET `" . $col_name . "` = ' WHERE " . $where_cond);
                $sql->bind_param("ss", addslashes($_POST['newName']), $param);
                $sql->execute();
        ...
        else // display form
        {
            ...
            $sql->execute();
            $tempResult = $sql->get_result();
            $oldName = $tempResult->fetch_assoc();
...
function display_path_content()
{
    ...
    global $_cid;
    global $langModule;
    global $imgRepositoryWeb, $conn;
    $style = "";
    $output = "";

    $sql = $conn->prepare("SELECT M.`name`, M.`contentType`,
        ...
        WHERE LP.`learnPath_id` = ?
        AND LP.`learnPath_id` = LPM.`learnPath_id`
        AND LPM.`module_id` = M.`module_id`
        ORDER BY LPM.`rank`");
    $sql->bind_param("sssss", $tbl_lp_learnPath, $tbl_lp_rel_learnPath_module, $tbl_lp_module, $tbl_lp_asset, $_SESSION['path_id']);
    $sql->execute();
    $moduleList = $sql->get_result();

    $extendedList = array();
    while ($module = $moduleList->fetch_assoc())
    {
        $extendedList[] = $module;
    }
...
function get_learnPath_progress($lpid, $lpUid)
{
    ...
    $sql = $conn->prepare("SELECT UMP.`raw` AS R, UMP.`scoreMax` AS SMax, M.`contentType` AS CTYPE, UMP.`lesson_status` AS STATUS
        FROM ? AS LP,
        ? AS LPM,
        ? AS UMP,
        ? AS M
        ...
        AND M.`contentType` != ?");
    $sql->bind_param("sssssss", $tbl_lp_learnPath, $tbl_lp_rel_learnPath_module, $tbl_lp_user_module_progress, $tbl_lp_module, $lpUid, $lpid, CTLABEL_);
    $sql->execute();
    $result = $sql->get_result();
    ...
    else
    {
        ...
        $sqlnum = $conn->prepare("SELECT COUNT(M.`module_id`)
            FROM ? AS LPM,
            ? AS M
            WHERE LPM.`learnPath_id` = ?
            AND LPM.`visibility` = 'SHOW'
            AND M.`contentType` != ?
            AND M.`module_id` = LPM.`module_id`
            ");
        $sqlnum->bind_param("ssss", $tbl_lp_rel_learnPath_module, $tbl_lp_module, $lpid, CTLABEL_);
        $sqlnum->execute();
        $result = $sqlnum->get_result();
        if($result) {
            list($value) = $result->fetch_assoc();
            $result->free_result();
...
function display_my_exercises($dialogBox, $style)
{
    ...
    global $imgRepositoryWeb, $langComment, $langSelection, $conn;
    $output = "";
    ...
    $atleastOne = FALSE;
    $sql = $conn->prepare("SELECT `id`, `titre` AS `title`, `description`
        FROM ?
        ORDER BY  `titre`, `id`");
    $sql->bind_param("s", $tbl_quiz_test);
    $sql->execute();
    $exercises = $sql->get_result();
    ...
    foreach($module_tree as $module)
    {
        if($module['visibility'] != $visibility)
        {
            $sql = $conn->prepare("UPDATE ?
                ...
                AND `visibility` != ?");
            $sql->bind_param("ssss", $tbl_lp_rel_learnPath_module, addslashes($visibility), $module['learnPath_module_id'], addslashes($visibility));
            $sql->execute();
        }
    ...
    foreach($module_tree as $module)
    {
        switch($module['contentType'])
        {
            ...
            case CTSCORM_ :
                ...
                $delAssetSql = $conn->prepare("DELETE
                    FROM ?
                    WHERE `module_id` = ?");
                $delAssetSql->bind_param("ss", $tbl_lp_asset, $module['module_id']);
                $delAssetSql->execute();
                // no break; because we need to delete modul
            case CTLABEL_ : // delete module if scorm && if label
                $delModSql = $conn->prepare("DELETE FROM ?
                WHERE `module_id` =  ?");
                $delModSql->bind_param("ss", $tbl_lp_module, $module['module_id']);
                $delModSql->execute();
                // no break; because we need to delete LMP and UMP
            default : // always delete LPM and UMP
                $tempSql = $conn->prepare("DELETE FROM ?
                    WHERE `learnPath_module_id` = ?");
                $tempSql->bind_param("ss", $tbl_lp_rel_learnPath_module, $module['learnPath_module_id']);
                $tempSql->execute();

                $tempSql = $conn->prepare("DELETE FROM ?
                    WHERE `learnPath_module_id` = ?");
                $tempSql->bind_param("ss", $tbl_lp_user_module_progress, $module['learnPath_module_id']);
                $tempSql->execute();
...
function get_limited_page_links($sql, $limiter, $stringPreviousPage, $stringNextPage)
{
    ...
    $totalnum = mysqli_num_rows($tempResult);
	$firstpage = 1;
