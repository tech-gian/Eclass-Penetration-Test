...
{
	global $currentCourseID, $urlServer, $conn;
	...
	if ($dirpath == '/') {
		...
		if ($dirpath != $oldpath) {
			...
			foreach ($components as $c) {
				$partial_path .= '/' . $c;
				$tempSql = $conn->prepare("SELECT filename FROM document WHERE path = ?");
				$tempSql->bind_param("s", $partial_path);
				$tempSql->execute();
				$q = $tempSql->get_result();
				list($name) = $q->fetch_assoc();
        ...
        if (!isset($filename)) {
				$tempSql = $conn->prepare("SELECT filename FROM document WHERE path = ?");
				$tempSql->bind_param("s", $path);
				$tempSql->execute();
				$q = $tempSql->get_result();
                list($filename) = $q->fetch_assoc();
        }
