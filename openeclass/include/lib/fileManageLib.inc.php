...
function update_db_info($dbTable, $action, $oldPath, $newPath = "")
{
	...
	if ($action == "delete") {
		...
		$tempSql->bind_param("ss", $dbTable, $oldPath);
		$tempSql->execute();
		
	} elseif ($action = "update") {
		$tempSql = $conn->prepare("UPDATE ? SET path = CONCAT(?, SUBSTRING(path, LENGTH(?)+1))
			WHERE path LIKE CONCAT(?, '%')");
		$tempSql->bind_param("ssss", $dbTable, $newPath, $oldPath, $oldPath);
		$tempSql->execute();
	}
}
...
{
	global $langParentDir, $langTo, $langMoveFrom, $langMove, $moveFileNameAlias, $conn;
	...
	if ($dirList)
	{
		while (list( , $pathValue) = each($dirList))
		{
			...
			$sql = $conn->prepare("SELECT path, filename FROM ? 
				WHERE path LIKE CONCAT('%', ?, '%')");
			$sql->bind_param("ss", $dbTable, $dirname);
			$sql->execute();
			$tempResult = $sql->get_result();
			while ($r = $tempResult->fetch_assoc()) {
				$filename = $r['filename'];
				$path = $r['path']; 
				$tab = "";	
